...
{
    explicit NotifierState(size_t capacity)
        : slots(capacity)
    {
    }
    sc_status Add(uint64_t key, std::shared_ptr<ReadinessSlot>& result)
    {
        ...
        return SC_TOO_LARGE; // 우편함이 찼다. 자리가 비는 것을 알릴 수단이 없으므로 WouldBlock이 아니다.
    }
    sc_status Next(uint32_t timeout, uint64_t* key)
    {
        ...
        *key = 0;
        ...
        if (!ready())
        {
            ...
            else if (!changed.wait_for(guard, std::chrono::milliseconds(timeout), ready))
                return SC_TIMEOUT;
        }
        if (closed)
            return SC_CLOSED;
...
public:
    ...
    {
        std::lock_guard guard(mMutex);
        for (const auto& value : mSlots)
            if (auto slot = value.lock())
                slot->Signal();
    }
...
struct sc_notifier
{
    std::shared_ptr<ServerCore::CDetail::NotifierState> state;
};
...
namespace ServerCore::CDetail
{
inline sc_status MakeSubscription(
    sc_notifier* notifier, uint64_t key, std::unique_ptr<sc_subscription>& result)
{
    if (!notifier)
        return SC_INVALID_ARGUMENT;
...
sc_status Subscribe(ReadinessSource& source, Ready&& ready, sc_notifier* notifier, uint64_t key,
    sc_subscription** out)
{
    if (!out)
        return SC_INVALID_ARGUMENT;
...
inline sc_status SubscribeCancellation(
    std::stop_token token, sc_notifier* notifier, uint64_t key, sc_subscription** out)
{
    if (!out)
        return SC_INVALID_ARGUMENT;
    *out = nullptr;
    ...
    if (status != SC_OK)
        return status;
    result->cancellation =
        std::make_unique<std::stop_callback<ReadinessStop>>(token, ReadinessStop{ result->slot });
    *out = result.release();
    return SC_OK;
}
template <class Register>
sc_status SubscribeCompletion(
    Register&& callback, sc_notifier* notifier, uint64_t key, sc_subscription** out)
{
    if (!out)
        return SC_INVALID_ARGUMENT;
    *out = nullptr;
    ...
    // 네이티브 등록은 관찰자 상한이 찼을 때만 WouldBlock을 돌려준다. C에서는 기다릴 수 없는 상한이므로 TooLarge다.
    if (!registered.IsOk())
    {
        const auto code = registered.GetStatus().Code();
        return code == Core::ErrorCode::WouldBlock ? SC_TOO_LARGE : static_cast<sc_status>(code);
    }
