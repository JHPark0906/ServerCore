...
extern "C"
{
    sc_status sc_atomic_file_options_init(sc_atomic_file_options* value, size_t size)
    {
    ...
    int sc_file_supports_directory_sync(void)
    {
        return Core::AtomicFile::SupportsDirectorySync();
    }
    sc_status sc_atomic_file_create(
        sc_bytes target, const sc_atomic_file_options* options, sc_atomic_file** out)
    {
        if (!out)
            return SC_INVALID_ARGUMENT;
        *out = nullptr;
    ...
    sc_status sc_atomic_file_write(sc_atomic_file* value, sc_bytes data)
    {
        return value && C::Valid(data)
                   ? C::Protect([&] { return C::Code(value->native->Write(C::Bytes(data))); })
                   : SC_INVALID_ARGUMENT;
    }
    sc_status sc_atomic_file_commit(sc_atomic_file* value)
    {
        return value ? C::Protect([&] { return C::Code(value->native->Commit()); })
                     : SC_INVALID_ARGUMENT;
    }
    int sc_atomic_file_committed(const sc_atomic_file* value)
    {
        return value && value->native->Committed();
    }
    uint64_t sc_atomic_file_written(const sc_atomic_file* value)
    {
        return value ? value->native->BytesWritten() : 0;
    }
    sc_status sc_atomic_file_cancel(sc_atomic_file* value)
    {
        return value ? C::Code(value->native->Cancel()) : SC_INVALID_ARGUMENT;
    }
    void sc_atomic_file_destroy(sc_atomic_file* value)
    {
        delete value;
    }
}
