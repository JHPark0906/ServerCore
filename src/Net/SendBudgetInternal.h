...
public:
    ...
    [[nodiscard]] SERVERCORE_TEST_API std::size_t UsedBytes() const noexcept;
    [[nodiscard]] SERVERCORE_TEST_API std::size_t ConnectionLimitBytes() const noexcept;
    SERVERCORE_TEST_API std::uint64_t Register(const std::shared_ptr<SendCapacityState>& state);
    SERVERCORE_TEST_API void Unregister(std::uint64_t id) noexcept;
    ...
    SERVERCORE_TEST_API void Notify() noexcept;
    SERVERCORE_TEST_API void MarkChanged() noexcept;
