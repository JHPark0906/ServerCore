...
{
SendBudget::SendBudget(const std::size_t limitBytes, const std::size_t connectionLimitBytes) noexcept
    : mLimitBytes(limitBytes), mConnectionLimitBytes(connectionLimitBytes)
{
...
std::size_t SendBudget::ConnectionLimitBytes() const noexcept { return mConnectionLimitBytes; }
void SendBudget::MarkChanged() noexcept { mChanged.store(true, std::memory_order_release); }
...
{
    if (!mChanged.load(std::memory_order_acquire) || mNotifying.test_and_set(std::memory_order_acquire)) return;
    for (;;)
    {
        mChanged.store(false, std::memory_order_release);
        std::uint64_t last = 0;
        std::uint64_t upper = 0;
        { const std::lock_guard guard(mWaitersMutex); upper = mNextId - 1; }
        for (;;)
        {
            std::shared_ptr<SendCapacityState> state;
            {
                ...
                if (next == mWaiters.end() || next->first > upper) break;
                last = next->first;
                state = next->second.lock();
            }
            if (state) state->Notify();
        }
        if (mChanged.load(std::memory_order_acquire)) continue;
