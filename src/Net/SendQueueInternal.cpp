...
Core::Status SendQueue::Enqueue(const std::span<const std::byte> bytes) noexcept
{
    ...
    if (mRetainedBytes > mBudget->ConnectionLimitBytes() || bytes.size() > mBudget->ConnectionLimitBytes() - mRetainedBytes)
...
std::shared_ptr<SendBudget> SendQueue::Budget() const noexcept { return mBudget; }
void SendQueue::CloseCapacityWaits() noexcept { mCapacity->Close(); }
Core::Result<SendCapacitySubscription> SendQueue::WaitForCapacity(std::size_t bytes,
