...
struct NativeEndpoint
{
    ...
    [[nodiscard]] const sockaddr* Address() const noexcept { return reinterpret_cast<const sockaddr*>(&storage); }
...
inline bool ToNativeEndpoint(const Core::IpEndpoint& endpoint,NativeEndpoint& output) noexcept
{
    ...
    {
        sockaddr_in address{};
        address.sin_family=AF_INET; address.sin_port=htons(endpoint.port);
        std::memcpy(&address.sin_addr,ip.Bytes().data(),4);
        std::memcpy(&value.storage,&address,sizeof address); value.length=sizeof address;
    }
    else if (ip.Family()==Core::IpFamily::V6)
    {
        sockaddr_in6 address{};
        address.sin6_family=AF_INET6; address.sin6_port=htons(endpoint.port); address.sin6_scope_id=ip.ScopeId();
...
inline Core::IpEndpoint FromNativeEndpoint(const sockaddr_storage& storage,SocketAddressLength length) noexcept
{
    if (storage.ss_family==AF_INET && length>=static_cast<SocketAddressLength>(sizeof(sockaddr_in)))
    {
        sockaddr_in address{}; std::memcpy(&address,&storage,sizeof address);
        auto ip=Core::IpAddress::FromBytes(Core::IpFamily::V4,
            {reinterpret_cast<const std::uint8_t*>(&address.sin_addr),4});
        return {ip.Value(),ntohs(address.sin_port)};
    }
    if (storage.ss_family==AF_INET6 && length>=static_cast<SocketAddressLength>(sizeof(sockaddr_in6)))
    {
        sockaddr_in6 address{}; std::memcpy(&address,&storage,sizeof address);
        auto ip=Core::IpAddress::FromBytes(Core::IpFamily::V6,
            {reinterpret_cast<const std::uint8_t*>(&address.sin6_addr),16},address.sin6_scope_id);
...
inline Core::IpEndpoint ReadSocketEndpoint(NativeSocket socket,bool remote) noexcept
{
    ...
    const auto result=remote ? ::getpeername(socket,reinterpret_cast<sockaddr*>(&storage),&length) :
...
inline Core::IpEndpoint FromNativeEndpoint(const sockaddr* address,SocketAddressLength length) noexcept
{
    if (!address || length<static_cast<SocketAddressLength>(sizeof(address->sa_family)) ||
        length>static_cast<SocketAddressLength>(sizeof(sockaddr_storage))) return {};
    sockaddr_storage storage{};
...
{
    if (family!=AF_INET6) return true;
