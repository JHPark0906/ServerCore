...
Core::Status DatagramSocket::Bind(const Core::IpEndpoint& binding, const bool ipv6Only)
{
    ...
    const int descriptor =
        ::socket(endpoint.Family(), SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC, IPPROTO_UDP);
    if (descriptor < 0)
        return MakePosixFailure("UDP socket", errno);
...
DatagramReceiveResult DatagramSocket::Receive(const std::span<std::byte> buffer) noexcept
{
    ...
    do
    {
        count = ::recvmsg(mSocket, &message, 0);
    ...
    if (count < 0)
    {
        ...
        return { MakePosixFailure("UDP recvmsg", error), {}, 0,
            error == ECONNRESET || error == ECONNREFUSED };
    }
    ...
    const Core::IpEndpoint& destination, const std::span<const std::byte> payload) noexcept
{
    ...
    do
    {
        count = ::sendto(mSocket, payload.empty() ? &empty : payload.data(), payload.size(),
            MSG_NOSIGNAL, endpoint.Address(), endpoint.length);
    } while (count < 0 && errno == EINTR);
    if (count < 0)
    {
        const int error = errno;
