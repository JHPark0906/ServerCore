...
{
public:
    explicit GroupScope(const TaskGroupState* state) noexcept : mState(state), mPrevious(currentGroupScope)
...
{
    if (const auto value = child.lock()) (void)value->cancellation.request_stop();
}
struct CancelGroup
{
...
public:
    ...
    {
        if (!closed || submitting != 0) return false;
        for (const auto& child : children)
            if (!child->published || !child->completed) return false;
        return true;
    }
    void NotifyCompletion() noexcept
    {
        bool complete;
        { const std::lock_guard guard(mutex); complete = FinishedLocked(); }
        wake.notify_all();
        if (complete) completionSignal.Complete();
    }
    void CloseAdmission() noexcept
    {
        { const std::lock_guard guard(mutex); closed = true; }
        NotifyCompletion();
    }
    ...
    Status Start(TaskExecutor& target, const TaskGroupOptions& configuration)
    {
        ...
        {
            const std::lock_guard guard(mutex);
            if (started) return Status::FailWithoutMessage(ErrorCode::Closed);
            started = true;
    ...
    Core::Result<TaskGroupTask> Submit(TaskExecutor::Task task, TaskOptions configuration)
    {
        ...
        const GroupScope scope(this);
        if (!task) return Result::FromStatus(Status::FailWithoutMessage(ErrorCode::InvalidArgument));
        ...
        try
        {
            ...
            if (!completion.IsOk()) return Result::FromStatus(std::move(completion).TakeStatus());
            ...
            if (options.deadline && (!configuration.deadline || *options.deadline < *configuration.deadline))
    ...
    Status WaitUntil(Clock::time_point deadline)
    {
        ...
        if (FinishedLocked()) return Status::Ok();
        if (GroupScope::Contains(this) || (executor && executor->IsCurrentThreadWorker()))
            return Status::FailWithoutMessage(ErrorCode::InvalidArgument);
        if (!wake.wait_until(guard, deadline, [this] { return FinishedLocked(); }))
...
{
    if (const auto state = group.lock()) state->Cancel();
}
}
TaskGroup::TaskGroup() : mState(std::make_shared<Detail::TaskGroupState>()) {}
TaskGroup::~TaskGroup()
{
    SERVERCORE_ASSERT(!mState->Forbidden(), "TaskGroup must be destroyed outside its executor and callbacks");
    (void)mState->Stop();
}
...
void TaskGroup::CloseAdmission() noexcept { mState->CloseAdmission(); }
...
void TaskGroup::RequestCancel() noexcept { mState->Cancel(); }
Core::Status TaskGroup::Wait() { return WaitUntil(std::chrono::steady_clock::time_point::max()); }
Core::Status TaskGroup::WaitUntil(std::chrono::steady_clock::time_point deadline) { return mState->WaitUntil(deadline); }
Core::Status TaskGroup::Stop() { return mState->Stop(); }
std::size_t TaskGroup::OutstandingCount() const noexcept
{ const std::lock_guard guard(mState->mutex); return mState->children.size(); }
