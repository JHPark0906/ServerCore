...
public:
    explicit KeyedScope(const KeyedExecutorState* value) noexcept
        : executor(value)
        , previous(currentKeyedScope)
    {
        currentKeyedScope = this;
    }
...
public:
    KeyedTaskState(std::uint64_t value, KeyedExecutor::Task function,
        ...
        : key(value)
        , job(std::move(function))
        , completion(std::move(observer))
...
public:
        ...
        KeyedExecutor::Completion completion, const TaskOptions& configuration)
    {
        ...
        try
        {
            task = std::make_shared<KeyedTaskState>(
                key, std::move(job), std::move(completion), configuration);
            task->owner = shared_from_this();
            if (configuration.parentToken.stop_possible())
                task->parentRegistration =
                    std::make_unique<std::stop_callback<KeyedParentCancellation>>(
    ...
    {
        {
            ...
            if (task->phase == KeyedPhase::Completing || task->phase == KeyedPhase::Terminal ||
                ...
                return false;
            task->cancelReason =
                task->options.deadline && KeyedClock::now() >= *task->options.deadline
                    ? ErrorCode::Timeout
                    : ErrorCode::Cancelled;
        }
        wake.notify_all();
    ...
    {
        {
            ...
            {
                if (task->phase == KeyedPhase::Completing)
                    continue;
                ObserveCancellation(*task, KeyedClock::now());
...
private:
    ...
    Status Admission(std::uint64_t key, std::size_t bytes) const noexcept
    {
        ...
        if (!ready || stopping)
            return Fail(ErrorCode::Closed);
        if (outstanding >= options.maxOutstandingTasks ||
            bytes > options.maxRetainedBytes - retained)
        ...
        if (slot.outstanding >= options.maxOutstandingTasksPerKey ||
            bytes > options.maxRetainedBytesPerKey - slot.bytes)
            return Fail(ErrorCode::WouldBlock);
        return Status::Ok();
    }
    ...
    {
        if (key.ready || key.busy || key.queued.empty())
            return;
        key.ready = true;
    ...
    void Run()
    {
        ...
        for (;;)
        {
            ...
            try
            {
                result = task->job(task->cancellation.get_token());
            }
            catch (...)
            {
                result = Fail(ErrorCode::PlatformError);
            }
            ErrorCode reason;
            {
                const std::lock_guard guard(mutex);
    ...
    void Coordinate()
    {
        ...
        for (;;)
        {
            ...
            if (selected)
            {
                ...
                if (queued)
                    Publish(selected, Fail(reason), false);
                guard.lock();
                continue;
            }
            if (stopping && tasks.empty())
                return;
            if (nextDeadline && *nextDeadline != KeyedClock::time_point::max())
...
void KeyedParentCancellation::operator()() const noexcept
{
    if (const auto value = task.lock())
        if (const auto owner = value->owner.lock())
            (void)owner->Cancel(value);
}
...
KeyedTaskHandle::KeyedTaskHandle(std::shared_ptr<Detail::KeyedTaskState> state) noexcept
    : mState(std::move(state))
{
}
bool KeyedTaskHandle::IsValid() const noexcept
{
...
KeyedExecutor::KeyedExecutor()
    : mState(std::make_shared<Detail::KeyedExecutorState>())
{
}
KeyedExecutor::~KeyedExecutor()
{
...
Core::Status KeyedExecutor::Start(const KeyedExecutorOptions& options)
{
    return mState->Start(options);
}
Core::Result<KeyedTaskHandle> KeyedExecutor::Submit(
    std::uint64_t key, Task task, const TaskOptions& options)
{
    return SubmitWithCompletion(key, std::move(task), {}, options);
}
Core::Result<KeyedTaskHandle> KeyedExecutor::SubmitWithCompletion(
...
void KeyedExecutor::RequestStop() noexcept
{
    mState->RequestStop();
}
Core::Status KeyedExecutor::Stop()
{
    return mState->Stop();
}
bool KeyedExecutor::IsStopRequested() const noexcept
{
    return mState->IsStopRequested();
}
std::size_t KeyedExecutor::KeyCount() const noexcept
{
    return mState->KeyCount();
}
std::size_t KeyedExecutor::PendingCount() const noexcept
{
    return mState->GetMetrics().pendingTasks;
}
std::size_t KeyedExecutor::RunningCount() const noexcept
{
    return mState->GetMetrics().runningTasks;
}
std::size_t KeyedExecutor::OutstandingCount() const noexcept
{
    return mState->OutstandingCount();
}
std::size_t KeyedExecutor::OutstandingCount(std::uint64_t key) const noexcept
{
    return mState->OutstandingCount(key);
}
std::size_t KeyedExecutor::RetainedBytes() const noexcept
{
    return mState->GetMetrics().retainedBytes;
}
std::size_t KeyedExecutor::RetainedBytes(std::uint64_t key) const noexcept
{
    return mState->RetainedBytes(key);
}
Observability::TaskExecutorMetricsSnapshot KeyedExecutor::GetMetrics() const noexcept
{
