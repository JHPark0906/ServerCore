...
SERVERCORE_TEST_API void InstallBeforeParseGate(std::shared_ptr<IBeforeParseGate> gate);

/// <summary>같은 gate가 아직 설치되어 있을 때만 제거한다.</summary>
/// <remarks>서로 독립적인 시험이 다른 gate를 설치했으면 그것을 지우지 않는다.</remarks>
SERVERCORE_TEST_API void ClearBeforeParseGate(const std::shared_ptr<IBeforeParseGate>& expected);
...
SERVERCORE_TEST_API void ClearFinalizePostFailure() noexcept;
}
