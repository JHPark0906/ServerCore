...
{
public:
    explicit State(std::function<void(Status)> callback)
        : mCallback(std::make_unique<std::function<void(Status)>>(std::move(callback)))
    {
    }
    ...
    bool Complete(ErrorCode code) noexcept
    {
        ...
        try
        {
            (*callback)(code == ErrorCode::Ok ? Status::Ok() : Status::FailWithoutMessage(code));
        }
        catch (...)
        {
        }
    ...
    struct CancelCallback
    {
        ...
        void operator()() const noexcept
        {
            if (auto value = state.lock())
                (void)value->Complete(ErrorCode::Cancelled);
        }
    };
...
CompletionSubscription::CompletionSubscription(std::shared_ptr<State> state) noexcept
...
CompletionSubscription::CompletionSubscription(CompletionSubscription&& other) noexcept
...
{
    if (!callback)
        return Result<CompletionSubscription>::FromStatus(
            Status::FailWithoutMessage(ErrorCode::InvalidArgument));
    try
    {
        return Result<CompletionSubscription>::FromValue(
            CompletionSubscription(std::make_shared<State>(std::move(callback))));
    }
    catch (...)
    {
        return Result<CompletionSubscription>::FromStatus(Status::AllocationFailure());
    }
}
CompletionSource CompletionSubscription::GetSource() const noexcept
{
    return CompletionSource(mState);
}
void CompletionSubscription::BindCancellation(
    std::stop_token first, std::stop_token second) noexcept
{
    if (const auto state = mState)
        state->BindCancellation(first, second);
}
void CompletionSubscription::Reset() noexcept
{
    if (auto state = std::move(mState))
        state->Reset();
}
bool CompletionSubscription::Cancel() noexcept
{
    const auto state = mState;
    return state && state->Complete(ErrorCode::Cancelled);
}
bool CompletionSubscription::IsPending() const noexcept
{
    return mState && mState->IsPending();
}
CompletionSource::CompletionSource(std::weak_ptr<CompletionSubscription::State> state) noexcept
    : mState(std::move(state))
{
}
bool CompletionSource::Complete(ErrorCode code) const noexcept
{
    ...
    return state && state->Complete(code);
}
