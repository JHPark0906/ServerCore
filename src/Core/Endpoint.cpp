...
bool V4(std::string_view text, std::uint8_t* bytes) noexcept
{
    for (std::size_t index = 0; index < 4; ++index)
    {
        ...
        unsigned value = 0;
        if (part.size() > 3 || (part.size() > 1 && part.front() == '0') || !Decimal(part, value) ||
            value > 255)
            return false;
...
bool Words(std::string_view text, bool allowV4, std::array<std::uint16_t, 8>& words,
    std::size_t& count) noexcept
{
    ...
    while (!text.empty())
    {
        ...
        if (part.empty() || count == 8)
            return false;
        if (part.contains('.'))
        {
            std::array<std::uint8_t, 4> bytes{};
            if (!allowV4 || colon != std::string_view::npos || count > 6 || !V4(part, bytes.data()))
                return false;
        ...
        for (const char digit : part)
        {
            unsigned hex;
            if (digit >= '0' && digit <= '9')
                hex = static_cast<unsigned>(digit - '0');
            else if (digit >= 'a' && digit <= 'f')
                hex = static_cast<unsigned>(digit - 'a' + 10);
            else if (digit >= 'A' && digit <= 'F')
                hex = static_cast<unsigned>(digit - 'A' + 10);
            else
                return false;
...
Result<IpAddress> IpAddress::FromBytes(
    IpFamily family, std::span<const std::uint8_t> bytes, std::uint32_t scope) noexcept
{
    if ((family != IpFamily::V4 && family != IpFamily::V6) ||
        bytes.size() != (family == IpFamily::V4 ? 4u : 16u) ||
        (family == IpFamily::V4 && scope != 0))
        return Invalid<IpAddress>();
    IpAddress value;
    value.mFamily = family;
    value.mScopeId = scope;
...
Result<IpAddress> IpAddress::Parse(std::string_view text) noexcept
{
    ...
    {
        if (!Decimal(text.substr(zone + 1), value.mScopeId))
            return Invalid<IpAddress>();
    ...
    {
        if (!Words(text, true, left, leftCount) || leftCount != 8)
            return Invalid<IpAddress>();
    }
    else
    {
        if (text.find("::", compressed + 2) != std::string_view::npos ||
            ...
            !Words(text.substr(compressed + 2), true, right, rightCount) ||
            leftCount + rightCount >= 8)
            return Invalid<IpAddress>();
        std::copy_n(
            right.begin(), rightCount, left.begin() + static_cast<std::ptrdiff_t>(8 - rightCount));
    }
...
std::string IpAddress::ToString() const
{
    ...
    for (std::size_t index = 0; index < 8; ++index)
        words[index] =
            static_cast<std::uint16_t>((unsigned(mBytes[index * 2]) << 8) | mBytes[index * 2 + 1]);
    ...
    for (std::size_t index = 0; index < 8;)
    {
        ...
        while (index < 8 && words[index] == 0)
            ++index;
        if (index - start >= 2 && index - start > length)
        {
            best = start;
            length = index - start;
        }
    ...
    for (std::size_t index = 0; index < 8;)
    {
        ...
        const auto converted =
            std::to_chars(digits.data(), digits.data() + digits.size(), words[index++], 16);
        result.append(digits.data(), converted.ptr);
    }
    if (mScopeId != 0)
    {
        result += '%';
        result += std::to_string(mScopeId);
    }
    return result;
}
Result<IpEndpoint> IpEndpoint::Parse(std::string_view text, std::uint16_t port) noexcept
{
    ...
    if (!address.IsOk())
        return Result<IpEndpoint>::FromStatus(std::move(address).TakeStatus());
...
std::string IpEndpoint::ToString() const
{
    if (!address.IsValid())
        return {};
    return (address.Family() == IpFamily::V6 ? "[" + address.ToString() + "]"
                                             : address.ToString()) +
           ":" + std::to_string(port);
}
Result<IpNetwork> IpNetwork::Parse(std::string_view text) noexcept
{
    ...
    if (slash == std::string_view::npos || text.contains('%'))
        return Invalid<IpNetwork>();
    auto address = IpAddress::Parse(text.substr(0, slash));
    unsigned prefix = 0;
    if (!address.IsOk() || !Decimal(text.substr(slash + 1), prefix) ||
        prefix > (address.Value().Family() == IpFamily::V4 ? 32u : 128u))
        return Invalid<IpNetwork>();
    IpNetwork value;
    value.mAddress = address.Value();
    value.mPrefix = static_cast<std::uint8_t>(prefix);
    for (std::size_t index = 0; index < value.mAddress.Bytes().size(); ++index)
    {
        const unsigned remaining =
            prefix > index * 8 ? prefix - static_cast<unsigned>(index * 8) : 0;
        const auto mask = remaining >= 8   ? 255u
                          : remaining == 0 ? 0u
                                           : (255u << (8 - remaining)) & 255u;
...
{
    if (!mAddress.IsValid() || address.Family() != mAddress.Family())
        return false;
    for (std::size_t index = 0; index < address.Bytes().size(); ++index)
    {
        const unsigned remaining =
            mPrefix > index * 8 ? mPrefix - static_cast<unsigned>(index * 8) : 0;
        const auto mask = remaining >= 8   ? 255u
                          : remaining == 0 ? 0u
                                           : (255u << (8 - remaining)) & 255u;
        if ((address.mBytes[index] & mask) != (mAddress.mBytes[index] & mask))
            return false;
    }
    return true;
}
