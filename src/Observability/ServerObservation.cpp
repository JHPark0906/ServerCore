...
void Drain(ObservationSnapshot& snapshot, std::uint64_t remaining) noexcept
{
    ...
    { snapshot.available |= DrainRemaining; snapshot.drainRemaining = remaining; }
}
}
ObservationSnapshot Observe(const HttpServerMetricsSnapshot& value) noexcept
{
    ObservationSnapshot result;
    result.protocol = Protocol::Http; result.lifecycle = value.lifecycle;
    ...
    result.receiveBytes = value.retainedRequestBytes; result.sendBytes = value.retainedSendBytes;
...
{
    ObservationSnapshot result;
    result.protocol = Protocol::Task; result.lifecycle = value.lifecycle;
    result.available = PendingWork | RetainedBytes | RejectedEvents | TimedOutEvents;
    result.pendingWork = Add(value.pendingTasks, value.runningTasks); result.retainedBytes = value.retainedBytes;
    result.rejected[0] = value.rejectedTasks; result.timedOut = value.timedOutTasks;
    Drain(result, result.pendingWork); return result;
}
ObservationSnapshot Observe(const LoggerMetricsSnapshot& value) noexcept
{
    ObservationSnapshot result;
    result.protocol = Protocol::Logger; result.lifecycle = value.lifecycle;
    ...
    result.retainedBytes = value.retainedBytes; result.rejected[0] = value.droppedMessages;
    Drain(result, result.pendingWork); return result;
}
ObservationSnapshot Observe(const Runtime::ServerMetricsSnapshot& value) noexcept
{
    ObservationSnapshot result;
    result.protocol = Protocol::Host; result.lifecycle = value.lifecycle;
    result.available = Connections | PendingWork | ReceiveBytes | SendBytes | RetainedBytes;
    ...
    result.retainedBytes = Add(Add(result.receiveBytes, result.sendBytes), value.jobs.retainedBytes);
    Drain(result, Add(result.connections, result.pendingWork)); return result;
}
ObservationSnapshot Observe(const Web::HttpServer& server) noexcept { return Observe(server.GetMetrics()); }
ObservationSnapshot Observe(const Runtime::TaskExecutor& executor) noexcept { return Observe(executor.GetMetrics()); }
ObservationSnapshot Observe(const AsyncLogger& logger) noexcept { return Observe(logger.GetMetrics()); }
ObservationSnapshot Observe(const Runtime::DatagramTransport& transport) noexcept
{
    const auto value = transport.SnapshotMetrics();
    ObservationSnapshot result; result.protocol = Protocol::Udp;
    result.lifecycle = value.bound ? Lifecycle::Running : Lifecycle::Unknown;
    result.available = PendingWork | RejectedEvents; result.pendingWork = value.pendingBatches;
    ...
    result.rejected[0] = Add(value.rejectedDatagrams > known ? value.rejectedDatagrams - known : 0, value.sendNotReady);
...
Core::Result<ObservationSnapshot> Observe(const Runtime::ServerHost& host)
{
    ...
    if (!metrics.IsOk()) return Core::Result<ObservationSnapshot>::FromStatus(std::move(metrics).TakeStatus());
