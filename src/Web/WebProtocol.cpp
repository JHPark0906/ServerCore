...
std::string_view HttpRequest::Header(std::string_view name) const noexcept
{
    ...
    {
        if (Detail::EqualInsensitive(field.first, name))
            return field.second;
    }
    return {};
}
...
/// <summary>쉼표로 나뉜 토큰 목록인지 본다.</summary>
/// <param name="allowEmpty">
/// 참이면 빈 원소("close,", "a,,b")를 건너뛴다(RFC 9110 §5.6.1.2). 프레이밍을 정하는
/// Transfer-Encoding은 거짓으로 불러 빈 원소를 계속 거절한다.
/// </param>
bool TokenList(std::string_view value, bool allowEmpty = false) noexcept
{
    while (true)
    {
        const std::size_t comma = value.find(',');
        const auto element = Trim(value.substr(0, comma));
        if (!IsToken(element) && !(allowEmpty && element.empty()))
            return false;
...
bool UpgradeProtocols(std::string_view value) noexcept
{
    while (true)
    {
        ...
        if (!IsToken(protocol.substr(0, slash)) ||
            (slash != std::string_view::npos && !IsToken(protocol.substr(slash + 1))))
            return false;
        if (comma == std::string_view::npos)
            return true;
        value.remove_prefix(comma + 1);
...
bool ParseHeader(std::string_view line, std::string& name, std::string& value)
{
    ...
    if (!ValidHeaderValue(fieldValue))
        return false;
...
bool ValidChunkExtensions(std::string_view value) noexcept
{
    ...
    while (true)
    {
        ...
        std::size_t count = 0;
        while (count < value.size() && IsToken(value.substr(count, 1)))
            ++count;
        if (count == 0)
            return false;
        ...
        if (value.front() == '"')
        {
            ...
            while (!value.empty())
            {
                ...
                if (ch == '\\')
                {
                    ...
                    if ((escaped < 32 && escaped != 9) || escaped == 127)
                        return false;
                    value.remove_prefix(1);
                }
                else if ((ch < 32 && ch != 9) || ch == 127)
                    return false;
            }
            if (!closed)
                return false;
...
std::string Base64(std::span<const std::byte> input)
{
    ...
    for (std::size_t offset = 0; offset < input.size(); offset += 3)
    {
        ...
        const auto second =
            offset + 1 < input.size() ? std::to_integer<unsigned int>(input[offset + 1]) : 0u;
        const auto third =
            offset + 2 < input.size() ? std::to_integer<unsigned int>(input[offset + 2]) : 0u;
        result.push_back(Base64Alphabet[first >> 2]);
        result.push_back(Base64Alphabet[((first & 3u) << 4) | (second >> 4)]);
        result.push_back(
            offset + 1 < input.size() ? Base64Alphabet[((second & 15u) << 2) | (third >> 6)] : '=');
...
std::array<std::byte, 20> Sha1(std::string_view text)
{
        ...
        for (std::size_t index = 0; index < 16; ++index)
        {
            for (std::size_t byte = 0; byte < 4; ++byte)
                words[index] = (words[index] << 8) |
                               std::to_integer<std::uint32_t>(bytes[offset + index * 4 + byte]);
        }
    ...
    for (std::size_t index = 0; index < hash.size(); ++index)
        for (std::size_t byte = 0; byte < 4; ++byte)
            result[index * 4 + byte] =
                static_cast<std::byte>((hash[index] >> ((3 - byte) * 8)) & 255u);
    return result;
}
}
...
bool IsToken(std::string_view value) noexcept
{
    ...
    for (const char ch : value)
    {
        if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9'))
            continue;
        if (!std::string_view("!#$%&'*+-.^_`|~").contains(ch))
            return false;
    }
    return true;
}
...
bool HasToken(const HttpRequest& request, std::string_view header, std::string_view token)
{
    ...
    {
        if (!EqualInsensitive(name, header))
            continue;
        std::string_view remaining(field);
        while (true)
        {
            const auto comma = remaining.find(',');
            if (EqualInsensitive(Trim(remaining.substr(0, comma)), token))
                return true;
            if (comma == std::string_view::npos)
                break;
...
HttpParser::HttpParser(std::size_t maxHeaders, std::size_t maxBody, bool separateHeaders)
    : mMaxHeaders(maxHeaders)
    , mMaxBody(maxBody)
    , mSeparateHeaders(separateHeaders)
{
}
...
bool HttpParser::Started() const noexcept
{
    return mStage != Stage::Headers;
}
bool HttpParser::IsHeadRequest() const noexcept
{
    return mHeadRequest;
}
...
HttpParseResult HttpParser::ParseHeaders(std::string_view block)
{
    ...
    if (firstSpace == std::string_view::npos || secondSpace == std::string_view::npos ||
        ...
        !IsToken(line.substr(0, firstSpace)))
        return Fail(400);
    const auto version = line.substr(secondSpace + 1);
    if (version != "HTTP/1.1" && version != "HTTP/1.0")
        return Fail(505);
    mHttp10 = version == "HTTP/1.0";
    auto target = line.substr(firstSpace + 1, secondSpace - firstSpace - 1);
    for (char ch : target)
        if (static_cast<unsigned char>(ch) < 33 || static_cast<unsigned char>(ch) >= 127 ||
            ch == '#')
            return Fail(400);
    // absolute-form(RFC 9112 §3.2.2)은 origin-form으로 줄이고, 그 authority를 Host 값으로 쓴다.
    std::string_view authority;
    if (!target.empty() && target.front() != '/' && target != "*")
    {
        const auto separator = target.find("://");
        if (separator == std::string_view::npos ||
            (!EqualInsensitive(target.substr(0, separator), "http") &&
                !EqualInsensitive(target.substr(0, separator), "https")))
            return Fail(400);
        const auto rest = target.substr(separator + 3);
        ...
        if (authority.empty() || authority.find_first_of(",@\\") != std::string_view::npos)
            return Fail(400);
        target = pathStart == std::string_view::npos ? std::string_view{} : rest.substr(pathStart);
        mRequest.target = target.starts_with('/') ? std::string{} : std::string("/");
        mRequest.target.append(target);
    }
    else
    {
        if (target.empty())
    ...
    while (!block.empty())
    {
        ...
        std::string name;
        std::string value;
        if (!ParseHeader(block.substr(0, end), name, value) || mRequest.headers.size() >= 128)
            return Fail(400);
        if (name == "host")
        {
            if (haveHost || value.empty() || value.find_first_of(" \t,/@?#\\") != std::string::npos)
                return Fail(400);
            haveHost = true;
        }
        else if (name == "content-length")
        {
            if (haveLength || value.empty())
                return Fail(400);
            for (char ch : value)
                if (ch < '0' || ch > '9')
                    return Fail(400);
            const auto parsed =
                std::from_chars(value.data(), value.data() + value.size(), mRemaining);
            if (parsed.ec != std::errc{} || mRemaining > mMaxBody)
                return Fail(413);
            haveLength = true;
        }
        else if (name == "transfer-encoding")
        {
            if (haveTransfer || !TokenList(value))
                return Fail(400);
            haveTransfer = true;
        ...
        {
            if (!TokenList(value, true))
                return Fail(400);
        }
        else if (name == "upgrade")
        {
            if (!UpgradeProtocols(value))
                return Fail(400);
        }
        else if (name == "expect")
        {
            if (haveExpect || !EqualInsensitive(value, "100-continue"))
                return Fail(417);
            haveExpect = true;
        }
    ...
    // HTTP/1.0은 Host를 요구하지 않고, Transfer-Encoding이 오면 프레이밍을 믿지 않는다(RFC 9112 §6.1).
    if ((!haveHost && !mHttp10) || (haveLength && haveTransfer) || (mHttp10 && haveTransfer))
        return Fail(400);
    if (transferFailure != 0)
        return Fail(transferFailure);
    if (!authority.empty())
    {
        const auto host = std::ranges::find(mRequest.headers, std::string_view("host"),
            [](const auto& field) -> std::string_view { return field.first; });
        if (host != mRequest.headers.end())
            host->second.assign(authority);
        else
            mRequest.headers.emplace_back("host", std::string(authority));
    }
    mStage = haveTransfer ? Stage::ChunkSize : Stage::FixedBody;
    // HTTP/1.0 클라이언트에게는 1xx를 보내지 않는다(RFC 9110 §10.1.1, §15.2).
    mContinue = haveExpect && !mHttp10 && (haveTransfer || mRemaining != 0);
    return {};
}
...
HttpParseResult HttpParser::Parse(std::string& input, HttpRequest& output)
{
    ...
    const auto result = Parse(std::string_view(input), consumed, output);
...
{
    auto rest = input;
    const auto result = Advance(rest, output);
    consumed = input.size() - rest.size();
    return result;
}

HttpParseResult HttpParser::Advance(std::string_view& input, HttpRequest& output)
{
    while (true)
    {
        if (mStage == Stage::Failed)
            return { HttpParseKind::Error, 400 };
        if (mStage == Stage::Headers)
        {
            // 요청 줄 앞의 빈 줄은 요청의 첫 바이트를 보기 전에만 건너뛴다(RFC 9112 §2.2).
            // 따로 도착한 CR은 LF를 기다린다. 건너뛴 줄은 requestTimeout이 묶는다.
            if (mHeaderSearch == 0)
            {
            ...
            // CR과 LF는 CRLF 짝으로만 받는다. 짝이 없는 LF나 CR은 머리 끝을 기다리지 않고 바로 400이다.
            // 새로 온 바이트만 훑으므로 조각난 도착에도 전체 비용은 입력 크기에 비례한다.
            auto end = std::string_view::npos;
            for (auto index = mHeaderSearch; index < input.size(); ++index)
            {
                const bool afterCr = index != 0 && input[index - 1] == '\r';
                if (input[index] == '\n')
                {
                    if (!afterCr)
                        return Fail(400);
                    if (index >= 3 && input[index - 2] == '\n')
                    {
                        end = index - 3;
                        break;
                    }
            ...
            if (end == std::string_view::npos)
            {
                if (input.size() >= mMaxHeaders)
                    return Fail(431);
                mHeaderSearch = input.size();
                return {};
            }
            if (end + 4 > mMaxHeaders)
                return Fail(431);
            const auto result = ParseHeaders(input.substr(0, end + 2));
            if (result.kind == HttpParseKind::Error)
                return result;
            ...
            if (mSeparateHeaders)
            {
                output = std::move(mRequest);
                mRequest = {};
                return { HttpParseKind::Headers, 200 };
            }
        }
        if (mContinue)
        {
        ...
        {
            const auto count =
                (std::min)({ input.size(), mRemaining, mStreaming ? mMaxChunk : mRemaining });
            mRequest.body.append(input.substr(0, count));
            input.remove_prefix(count);
            mRemaining -= count;
            mBodyBytes += count;
            if (mStreaming && count != 0)
            {
                output.body = std::move(mRequest.body);
                mRequest.body.clear();
        ...
        if (mStage == Stage::ChunkSize)
        {
            ...
            if (end == std::string_view::npos)
            {
                if (input.size() > mMaxHeaders - mChunkOverhead)
                    return Fail(431);
                return {};
            }
            if (end + 2 > mMaxHeaders - mChunkOverhead)
                return Fail(431);
            ...
            if (digits.empty() ||
                (hexEnd != std::string_view::npos && !ValidChunkExtensions(line.substr(hexEnd))))
                return Fail(400);
            const auto result =
                std::from_chars(digits.data(), digits.data() + digits.size(), mRemaining, 16);
            if (result.ec != std::errc{} || mRemaining > mMaxBody - mBodyBytes)
                return Fail(413);
        ...
        if (mStage == Stage::ChunkEnd)
        {
            ...
            if (!input.starts_with("\r\n"))
                return Fail(400);
            if (mChunkOverhead > mMaxHeaders - 2)
                return Fail(431);
            mChunkOverhead += 2;
            input.remove_prefix(2);
            if (mStreaming)
        ...
        if (mStage == Stage::Trailers)
        {
            ...
            if (end == std::string_view::npos)
            {
                if (input.size() > mMaxHeaders - mTrailerBytes)
                    return Fail(431);
                return {};
            }
            if (end + 2 > mMaxHeaders - mTrailerBytes)
                return Fail(431);
            mTrailerBytes += end + 2;
            if (end == 0)
            {
            ...
            std::string name;
            std::string value;
            if (!ParseHeader(input.substr(0, end), name, value) || ForbiddenTrailer(name))
                return Fail(400);
...
std::string HttpDate(std::chrono::system_clock::time_point time)
{
    ...
    const int written = std::snprintf(text.data(), text.size(),
        "%s, %02u %s %04d %02lld:%02lld:%02lld GMT", daysOfWeek[weekDay.c_encoding()],
        static_cast<unsigned int>(date.day()), months[static_cast<unsigned int>(date.month()) - 1],
        static_cast<int>(date.year()), static_cast<long long>(timeOfDay.hours().count()),
...
bool SerializeResponse(const HttpResponse& response, bool head, bool close, std::size_t maxHeaders,
    std::size_t maxBody, std::string& output)
{
    if (response.body.size() > maxBody)
        return false;
    ...
    if (prepared.Value().mode != ResponseBodyMode::None)
...
{
    if (key.size() != 24 || key.substr(22) != "==")
        return false;
    for (std::size_t index = 0; index < 22; ++index)
        if (!Base64Alphabet.contains(key[index]))
            return false;
    if ((Base64Alphabet.find(key[21]) & 15u) != 0)
        return false;
    std::string text(key);
...
FrameParseResult ParseClientFrame(
    std::span<const std::byte> input, std::size_t maxPayload, WebSocketFrame& frame)
{
    ...
    if (control && ((first & 0x80u) == 0 || (second & 127u) > 125))
        return { FrameParseKind::Error, 0, 1002 };
    ...
    if (length == 126)
    {
        if (input.size() < 4)
            return {};
        length = (std::to_integer<std::uint64_t>(input[2]) << 8) |
                 std::to_integer<std::uint64_t>(input[3]);
        if (length < 126)
            return { FrameParseKind::Error, 0, 1002 };
        header = 4;
    }
    else if (length == 127)
    {
        if (input.size() < 10)
            return {};
        if ((std::to_integer<unsigned int>(input[2]) & 128u) != 0)
            return { FrameParseKind::Error, 0, 1002 };
        length = 0;
    ...
    if (!control && length > maxPayload)
        return { FrameParseKind::Error, 0, 1009 };
    if (length > (std::numeric_limits<std::size_t>::max)() - header - 4)
        return { FrameParseKind::Error, 0, 1009 };
    const auto payloadSize = static_cast<std::size_t>(length);
    if (input.size() < header + 4 + payloadSize)
        return {};
...
bool SelectWebSocketSubprotocol(
    const HttpRequest& request, const std::vector<std::string>& supported, std::string& selected)
{
    ...
    for (const auto& [name, value] : request.headers)
    {
        ...
        for (;;)
        {
            ...
            if (!IsToken(token) || offered.size() == 64 || token.size() > 4096 - offeredBytes ||
                std::ranges::contains(offered, token))
                return false;
    ...
    for (const auto& token : supported)
        if (std::ranges::contains(offered, token))
        {
            selected = token;
            break;
        }
...
bool Utf8FragmentState::Append(std::span<const std::byte> bytes, bool final) noexcept
{
    ...
    const auto validPartial = [&]()
    {
        ...
        for (auto index = size; index < expected; ++index)
            padded[index] = static_cast<char>(
                index == 1 && static_cast<unsigned char>(pending[0]) == 0xe0   ? 0xa0
                : index == 1 && static_cast<unsigned char>(pending[0]) == 0xf0 ? 0x90
                                                                               : 0x80);
    ...
    if (size != 0)
    {
        ...
        if (size != expected)
            return !final && validPartial();
        if (!Core::Detail::IsValidUtf8(std::string_view(pending, size)))
            return false;
        size = 0;
    }
    if (!text.empty())
...
std::vector<std::byte> EncodeServerFrame(
    std::uint8_t opcode, std::span<const std::byte> payload, bool final)
{
    ...
    else
    {
        ...
        for (int shift = 56; shift >= 0; shift -= 8)
            result.push_back(static_cast<std::byte>((length >> shift) & 255u));
    }
