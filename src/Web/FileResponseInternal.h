...
inline bool ValidEntityTag(std::string_view tag) noexcept
{
    ...
    for (const unsigned char byte : tag)
        if (byte < 0x21 || byte == 0x22 || byte == 0x7f)
            return false;
    return true;
}
inline bool EntityTagMatches(std::string_view list, std::string_view actual, bool weak) noexcept
{
    ...
    while (!list.empty())
    {
        ...
        if (list.size() <= prefix || list[prefix] != '"')
            return false;
        ...
        if (tag == target && (weak || (!tag.starts_with("W/") && !target.starts_with("W/"))))
            matched = true;
...
inline std::optional<std::chrono::sys_seconds> ParseFileDate(std::string_view value)
{
        ...
        { "%a, %d %b %Y %H:%M:%S GMT", "%A, %d-%b-%y %H:%M:%S GMT", "%a %b %d %H:%M:%S %Y" })
    {
        ...
        if (input.fail() || input.peek() != std::char_traits<char>::eof() || parsed.tm_sec > 59)
            continue;
        ...
        if (year < 1601 || year > 9999 || !date.ok() || parsed.tm_hour < 0 || parsed.tm_hour > 23 ||
            parsed.tm_min < 0 || parsed.tm_min > 59 || parsed.tm_sec < 0)
            continue;
    ...
    std::string_view entityTag, std::chrono::system_clock::time_point modified)
{
    ...
    if (match && !EntityTagMatches(*match, entityTag, false))
        plan.status = 412;
    if (!match)
        if (const auto value = field("if-unmodified-since"))
            if (const auto date = ParseFileDate(*value); date && lastModified > *date)
                plan.status = 412;
    const auto noneMatch = field("if-none-match");
    if (plan.status == 200 && noneMatch && EntityTagMatches(*noneMatch, entityTag, true))
        plan.status = retrieval ? 304u : 412u;
    ...
    if (request.method != "GET")
        return plan; // Range has GET semantics only.
    ...
    if (const auto condition = field("if-range"))
    {
        ...
        if (!ValidEntityTag(*condition) || condition->starts_with("W/") ||
            entityTag.starts_with("W/") || *condition != entityTag)
            return plan;
    }
    auto value = FileTrim(*range);
    if (value.size() < 6 || !EqualInsensitive(value.substr(0, 6), "bytes="))
        return plan;
    value.remove_prefix(6);
    if (value.find(',') != std::string_view::npos)
        return plan; // Multipart is deliberately ignored.
    ...
    if (left.empty())
    {
        if (!integer(right, last))
            return plan;
        if (last == 0 || size == 0)
            plan.status = 416;
        else
        {
            plan.offset = size - (std::min)(size, last);
            plan.length = size - plan.offset;
            plan.status = 206;
        }
    }
    else
    {
        if (!integer(left, first) || (!right.empty() && !integer(right, last)))
            return plan;
        if (!right.empty() && first > last)
            return plan;
        if (first >= size)
            plan.status = 416;
        else
    ...
    {
        plan.length = 0;
        plan.contentRange = "bytes */" + std::to_string(size);
    }
    if (plan.status == 206)
        plan.contentRange = "bytes " + std::to_string(plan.offset) + "-" +
                            std::to_string(plan.offset + plan.length - 1) + "/" +
                            std::to_string(size);
    return plan;
}
}
