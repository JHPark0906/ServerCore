...
void MergePolicyHeaders(HttpHeaders& headers, const HttpHeaders& policy)
{
    ...
    {
        const auto same = [&](const auto& entry)
        {
            return EqualInsensitive(entry.first, field.first) &&
                   (!EqualInsensitive(field.first, "vary") ||
                       EqualInsensitive(entry.second, field.second));
        };
        if (EqualInsensitive(field.first, "set-cookie") ||
            std::none_of(headers.begin(), headers.end(), same))
...
HttpResponseState::HttpResponseState(std::shared_ptr<TrackedSender> sender,
    ...
    std::shared_ptr<RequestObservation> observation, HttpHeaders policyHeaders, bool closeDelimited,
    std::function<void()> finished)
    : mSender(std::move(sender))
    , mOptions(options)
    , mHead(head)
    , mRequestClose(close || closeDelimited)
    , mConnect(connect)
    , mCloseDelimited(closeDelimited)
    , mCallbackOwner(callbackOwner)
    , mCreated(WebNow())
    , mLastWrite(mCreated)
    , mHandlerStarted(mCreated)
    , mReservation(std::move(reservation))
    , mObservation(std::move(observation))
    , mPolicyHeaders(std::move(policyHeaders))
    ...
    std::uint64_t length, std::uint64_t written, bool close, unsigned status)
{
    ...
    {
        if (!wire.empty())
            result = mSender->Send(Bytes(wire));
    }
    catch (...)
    {
        result = Status::AllocationFailure();
    }
    bool drain = false;
    {
        const std::lock_guard guard(mMutex);
        mSending = false;
        if (mPhase == Phase::Aborted)
    ...
    if (drain)
        mSender->connection->CloseAfterSend();
    if (!result.IsOk() && result.Code() != ErrorCode::WouldBlock)
        AbortWithReason(result.Code());
    return result;
}
...
Status HttpResponseState::Start(const HttpResponseHead& value)
{
    ...
    {
        const std::lock_guard guard(mMutex);
        if (mPhase == Phase::Finished || mPhase == Phase::Aborted)
            return Fail(ErrorCode::Closed);
        if (mSending)
            return Fail(ErrorCode::WouldBlock);
        if (mPhase != Phase::New)
            return Fail(ErrorCode::AlreadyExists);
        if (mConnect && value.status >= 200 && value.status < 300)
            return Fail(ErrorCode::InvalidArgument);
        auto head = value;
        MergePolicyHeaders(head.headers, mPolicyHeaders);
        head.close = head.close || mRequestClose;
        auto prepared = PrepareResponseHead(head, mHead, mOptions.maxHeaderBytes, !mCloseDelimited);
        if (!prepared.IsOk())
            return std::move(prepared).TakeStatus();
        wire = std::move(prepared.Value().wire);
        mode = prepared.Value().mode;
        length = prepared.Value().length;
        close = head.close || mode == ResponseBodyMode::CloseDelimited;
        mSending = true;
    }
    catch (...)
    {
...
Status HttpResponseState::Write(std::span<const std::byte> bytes)
{
    ...
    try
    {
        ...
        if (mSending)
            return Fail(ErrorCode::WouldBlock);
        if (mPhase != Phase::Streaming)
            return Fail(ErrorCode::InvalidArgument);
        if (bytes.empty())
            return Status::Ok();
        if (mMode == ResponseBodyMode::None)
            return Fail(ErrorCode::InvalidArgument);
        ...
        if (mMode == ResponseBodyMode::Chunked)
        {
            ...
            wire.assign(reinterpret_cast<const char*>(bytes.data()), bytes.size());
        mode = mMode;
        length = mLength;
        written = mWritten + bytes.size();
        close = mClose;
        mSending = true;
    }
    catch (...)
...
Status HttpResponseState::Finish()
{
    ...
    try
    {
        ...
        if (mPhase != Phase::Streaming)
            return Fail(ErrorCode::InvalidArgument);
        if (mMode == ResponseBodyMode::FixedLength && mWritten != mLength)
            return Fail(ErrorCode::InvalidArgument);
        if (mMode == ResponseBodyMode::Chunked)
            wire = "0\r\n\r\n";
        mode = mMode;
...
Status HttpResponseState::Complete(const HttpResponse& response)
{
    ...
    try
    {
        ...
        if (response.body.size() > mOptions.maxResponseBodyBytes)
            return Fail(ErrorCode::TooLarge);
        if (mConnect && response.status >= 200 && response.status < 300)
            return Fail(ErrorCode::InvalidArgument);
        close = response.close || mRequestClose;
        auto actual = response;
        MergePolicyHeaders(actual.headers, mPolicyHeaders);
        if (!SerializeResponse(
                actual, mHead, close, mOptions.maxHeaderBytes, mOptions.maxResponseBodyBytes, wire))
            return Fail(ErrorCode::InvalidArgument);
        mSending = true;
    }
    catch (...)
    ...
    return SendTransaction(
        std::move(wire), Phase::Finished, ResponseBodyMode::None, 0, 0, close, response.status);
}
...
void HttpResponseState::Report(ErrorCode reason) noexcept
{
    ...
    if (mObservation)
        mObservation->Finish(reason, status);
}
...
std::stop_token HttpResponseState::GetCancellationToken() const noexcept
{
...
{
    const auto maximum =
        (std::min)(mOptions.maxTotalSendQueueCapacityBytes, Net::SendQueueLimitBytes);
    return (std::min)(mOptions.maxStreamChunkBytes, maximum > 32 ? maximum - 32 : std::size_t{ 0 });
}
std::size_t HttpResponseState::RetainedSendBytes() const noexcept
{
    return mSender->flow->RetainedSendBytes();
}
bool HttpResponseState::IsFinished() const noexcept
{
    const std::lock_guard guard(mMutex);
    return mPhase == Phase::Finished && mWaitsRetired;
}
bool HttpResponseState::IsAborted() const noexcept
{
    const std::lock_guard guard(mMutex);
    return mPhase == Phase::Aborted;
}
bool HttpResponseState::ShouldClose() const noexcept
{
    const std::lock_guard guard(mMutex);
    return mClose;
}
bool HttpResponseState::HasStarted() const noexcept
{
    const std::lock_guard guard(mMutex);
    return mPhase != Phase::New;
}
bool HttpResponseState::TimedOut(std::int64_t now, bool handlerDeadline) const noexcept
{
    const std::lock_guard guard(mMutex);
    return (handlerDeadline && mPhase == Phase::New &&
               now - mHandlerStarted >= mOptions.handlerTimeout.count()) ||
           (mPhase == Phase::Streaming && now - mLastWrite >= mOptions.streamIdleTimeout.count());
}
void HttpResponseState::RestartHandlerDeadline() noexcept
{
    const auto now = WebNow();
    const std::lock_guard guard(mMutex);
    mHandlerStarted = now;
}
    ...
    std::size_t bytes, std::function<void(Status)> callback)
{
    ...
    std::size_t required;
    {
        const std::lock_guard guard(mMutex);
        if (!callback)
            return Result::FromStatus(Fail(ErrorCode::InvalidArgument));
        if (mPhase == Phase::Finished || mPhase == Phase::Aborted)
            return Result::FromStatus(Fail(ErrorCode::Closed));
        if (bytes > MaxWriteBytes())
            return Result::FromStatus(Fail(ErrorCode::TooLarge));
        ...
        if (mSending)
            return Result::FromStatus(Fail(ErrorCode::WouldBlock));
        required =
            ((mPhase == Phase::New || bytes == 0) && mBlockedWireBytes != 0)
                ? mBlockedWireBytes
                : (mPhase == Phase::New
                          ? (std::min)(
                                mOptions.maxHeaderBytes, mOptions.maxTotalSendQueueCapacityBytes)
                          : (bytes == 0 ? std::size_t{ 5 }
                                        : bytes + (mMode == ResponseBodyMode::Chunked ? 32 : 0)));
    }
