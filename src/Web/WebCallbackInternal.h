...
{
public:
    WebCompletionScope() noexcept
        : mPrevious(activeCompletionScope)
    {
        activeCompletionScope = this;
    }
    ~WebCompletionScope()
    {
        ...
        while (mFirst)
        {
            auto item = std::move(mFirst);
            mFirst = std::move(item->next);
...
{
public:
    explicit WebCallbackScope(const void* owner) noexcept
        : mOwner(owner)
        , mPrevious(activeWebScope)
    {
        activeWebScope = this;
    }
...
inline void WebCompletionScope::Complete(std::shared_ptr<WebCompletion> item) noexcept
{
    ...
    if (auto* scope = activeCompletionScope)
    {
        auto* tail = item.get();
        if (scope->mLast)
            scope->mLast->next = std::move(item);
        else
            scope->mFirst = std::move(item);
        scope->mLast = tail;
    }
    else
    {
