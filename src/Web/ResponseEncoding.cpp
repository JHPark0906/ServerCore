...
Core::Result<PreparedResponseHead> PrepareResponseHead(
    const HttpResponseHead& response, bool head, std::size_t maxHeaders, bool chunkedAllowed)
{
    return PrepareResponseHead(response.status, response.headers, response.contentLength, head,
        response.close, maxHeaders, chunkedAllowed);
}
...
Core::Result<PreparedResponseHead> PrepareResponseHead(unsigned int status,
    ...
    std::size_t maxHeaders, bool chunkedAllowed)
{
    ...
    try
    {
        ...
        for (const auto& [name, value] : headers)
        {
            if (!IsToken(name) || !ValidHeaderValue(value) ||
                EqualInsensitive(name, "content-length") ||
                EqualInsensitive(name, "transfer-encoding") ||
                EqualInsensitive(name, "connection") || EqualInsensitive(name, "trailer"))
                return Result::FromStatus(Status::FailWithoutMessage(ErrorCode::InvalidArgument));
            ...
            if (!Append(prepared.wire, name, maxHeaders) ||
                !Append(prepared.wire, ": ", maxHeaders) ||
                !Append(prepared.wire, value, maxHeaders) ||
                !Append(prepared.wire, "\r\n", maxHeaders))
                return Result::FromStatus(Status::FailWithoutMessage(ErrorCode::TooLarge));
        }
        if (status == 426 && !haveUpgrade)
            return Result::FromStatus(Status::FailWithoutMessage(ErrorCode::InvalidArgument));
        if (!haveDate &&
            (!Append(prepared.wire, "Date: ", maxHeaders) ||
        ...
        prepared.mode = head || noContent ? ResponseBodyMode::None
                        : length          ? ResponseBodyMode::FixedLength
                        : chunkedAllowed  ? ResponseBodyMode::Chunked
                                          : ResponseBodyMode::CloseDelimited;
        // 연결 종료가 본문 끝이므로 닫지 않는 CloseDelimited 응답은 만들지 않는다.
        if (prepared.mode == ResponseBodyMode::CloseDelimited)
            close = true;
        ...
        if (status == 205 || (status != 204 && status != 304 && length))
        {
            ...
            return Result::FromStatus(Status::FailWithoutMessage(ErrorCode::TooLarge));
        if (!Append(prepared.wire, close ? "Connection: close" : "Connection: keep-alive",
                maxHeaders) ||
...
Core::Result<std::string> EncodeChunk(std::span<const std::byte> bytes, std::size_t maxChunkBytes)
{
    ...
    const auto encoded =
        std::to_chars(digits.data(), digits.data() + digits.size(), bytes.size(), 16);
    const auto digitCount = static_cast<std::size_t>(encoded.ptr - digits.data());
    if (encoded.ec != std::errc{} ||
        bytes.size() > (std::numeric_limits<std::size_t>::max)() - digitCount - 4)
        return Result::FromStatus(Status::FailWithoutMessage(ErrorCode::TooLarge));
    try
    {
        std::string wire;
