...
bool DecodeSegment(const std::string_view raw, std::string& output)
{
    ...
    for (std::size_t index = 0; index < raw.size(); ++index)
    {
        ...
        if (value < 33 || value >= 127 || value == '?' || value == '#') return false;
        if (value == '%')
        {
            if (raw.size() - index < 3) return false;
    ...
    if (decoded == "." || decoded == ".." || !Core::Detail::IsValidUtf8(decoded)) return false;
    output = std::move(decoded);
    return true;
}

template <typename Visitor>
bool VisitSegments(std::string_view path, Visitor&& visit)
{
    ...
    while (true)
    {
        ...
        if (!visit(path.substr(0, slash))) return false;
...
{
    if (name.empty() || !IdentifierStart(static_cast<unsigned char>(name.front()))) return false;
    for (const unsigned char value : name)
        if (!IdentifierStart(value) && (value < '0' || value > '9')) return false;
    return true;
}
}
...
bool CompileRoutePattern(const std::string_view pattern, RoutePattern& output)
{
    ...
    const bool valid = VisitSegments(pattern, [&](const std::string_view raw) {
        ...
        if (raw.find_first_of("{}") != std::string_view::npos)
        {
            ...
            if (!ValidParameterName(name) || !names.emplace(name).second) return false;
            segment.value.assign(name);
            segment.parameter = true;
        }
        else if (!DecodeSegment(raw, segment.value)) return false;
...
{
    RoutePath decoded;
    const bool valid = VisitSegments(rawPath, [&](const std::string_view raw) {
        std::string segment;
        if (!DecodeSegment(raw, segment)) return false;
...
{
    if (pattern.segments.size() != path.size()) return false;
    for (std::size_t index = 0; index < path.size(); ++index)
    {
        const auto& segment = pattern.segments[index];
        if (segment.parameter ? path[index].empty() : segment.value != path[index]) return false;
    }
    return true;
}
...
bool SameRouteShape(const RoutePattern& left, const RoutePattern& right) noexcept
{
    ...
    for (std::size_t index = 0; index < left.segments.size(); ++index)
    {
        ...
        if (leftSegment.parameter != rightSegment.parameter ||
            (!leftSegment.parameter && leftSegment.value != rightSegment.value)) return false;
    }
    return true;
}
...
{
    SERVERCORE_ASSERT(MatchesRoutePattern(pattern, path), "route parameters require a matching path");
