...
{
public:
    SERVERCORE_TEST_API HttpResponseState(std::shared_ptr<TrackedSender> sender, const HttpServerOptions& options,
    ...
    SERVERCORE_TEST_API bool TimedOut(std::int64_t now) const noexcept;
private:
    ...
    SERVERCORE_TEST_API Core::Status SendTransaction(std::string wire, Phase next, ResponseBodyMode mode,
