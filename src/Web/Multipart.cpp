...
Core::Result<Parameterized> Parameters(std::string_view input)
{
    ...
    result.main = Lower(Trim(input.substr(0, semicolon)));
    ...
    while (!input.empty())
    {
        ...
        if (equal == input.npos)
            return Fail<Parameterized>(ErrorCode::InvalidFormat);
        ...
        else
        {
            ...
            const auto token = Trim(input.substr(0, next));
            if (!Detail::IsToken(token))
                return Fail<Parameterized>(ErrorCode::InvalidFormat);
            value = token;
            input = next == input.npos ? std::string_view{} : input.substr(next + 1);
            if (next != input.npos && Trim(input).empty())
                return Fail<Parameterized>(ErrorCode::InvalidFormat);
        }
        if (!Metadata(value))
            return Fail<Parameterized>(ErrorCode::InvalidFormat);
...
{
    if (value.empty() || value.size() > 70 || value.back() == ' ')
        return false;
    for (const unsigned char ch : value)
        if (!((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') ||
                std::string_view("'()+_,-./:=? ").contains(static_cast<char>(ch))))
            return false;
    return true;
}
}
...
public:
    ...
    Suffix DelimiterSuffix(std::size_t start, std::size_t& end) const
    {
        ...
        while (pos < buffer.size() && (buffer[pos] == ' ' || buffer[pos] == '\t'))
            ++pos;
        if (pos - start > options.maxHeaderBytes)
            return Suffix::Invalid;
        if (pos == buffer.size())
        {
            if (final && eof)
        ...
        return final ? Suffix::Final : Suffix::Normal;
    }
    Core::Result<std::shared_ptr<const MultipartEvent>> Event(
        MultipartEvent event, std::size_t consumed)
    {
        std::size_t bytes = event.data.size() + event.name.size() + event.contentType.size();
        if (event.filename)
            bytes += event.filename->size();
        for (const auto& [name, value] : event.headers)
            bytes += name.size() + value.size();
        const auto held = credit->bytes.load();
        if (bytes > options.maxRetainedBytes)
        {
            FailWith(ErrorCode::TooLarge);
            return Fail<std::shared_ptr<const MultipartEvent>>(error);
        }
        ...
        std::shared_ptr<const MultipartEvent> result(owned.release(),
            [account = credit, bytes](const MultipartEvent* value)
            {
                delete value;
    ...
    Core::Result<std::shared_ptr<const MultipartEvent>> Data(std::size_t count)
    {
        ...
        if (count > limit - partBytes)
        {
            FailWith(ErrorCode::TooLarge);
            return Fail<std::shared_ptr<const MultipartEvent>>(error);
        }
        MultipartEvent event;
        event.kind = MultipartEventKind::Data;
        event.partIndex = part;
        event.data.assign(reinterpret_cast<const std::byte*>(buffer.data()),
            reinterpret_cast<const std::byte*>(buffer.data()) + count);
    ...
    Core::Result<MultipartEvent> Headers(std::string_view block)
    {
        ...
        while (!block.empty())
        {
            ...
            if (colon == line.npos || !Detail::IsToken(line.substr(0, colon)))
                return Fail<MultipartEvent>(ErrorCode::InvalidFormat);
            auto name = Lower(line.substr(0, colon));
            const auto value = Trim(line.substr(colon + 1));
            if (!Detail::ValidHeaderValue(value))
                return Fail<MultipartEvent>(ErrorCode::InvalidFormat);
            if (event.headers.size() == options.maxHeaders)
                return Fail<MultipartEvent>(ErrorCode::TooLarge);
            if (name == "content-disposition")
            {
                ...
                if (!parameters.IsOk())
                    return Core::Result<MultipartEvent>::FromStatus(
                        std::move(parameters).TakeStatus());
                if (parameters.Value().main != "form-data")
                    return Fail<MultipartEvent>(ErrorCode::InvalidFormat);
                bool named = false;
    ...
    Core::Result<std::shared_ptr<const MultipartEvent>> Read()
    {
        ...
        for (;;)
        {
            ...
            if (phase == Phase::Headers)
            {
                ...
                if (!parsed.IsOk())
                {
                    FailWith(parsed.GetStatus().Code());
                    return Result::FromStatus(std::move(parsed).TakeStatus());
                }
                const bool nextFile = parsed.Value().filename.has_value();
                if (nextFile && files == options.maxFiles)
                {
                    FailWith(ErrorCode::TooLarge);
                    return Fail<std::shared_ptr<const MultipartEvent>>(error);
                }
            ...
            for (;;)
            {
                ...
                if (initial && initialStart && buffer.size() < marker.size() &&
                    marker.starts_with(buffer))
                    break;
                if (initial && initialStart && buffer.starts_with(marker))
                {
                    at = 0;
                ...
                {
                    at = buffer.find(delimiter, search);
                    markerSize = delimiter.size();
                }
                if (at == buffer.npos)
                {
                    ...
                    if (buffer.size() > keep)
                    {
                        ...
                        if (!initial)
                            return Data(discard);
                        buffer.erase(0, discard);
                        initialStart = false;
                    }
                    break;
                }
                std::size_t end = 0;
                const auto suffix = DelimiterSuffix(at + markerSize, end);
                if (suffix == Suffix::Invalid)
                {
                    if (initial && initialStart && at == 0)
                        initialStart = false;
                    search = at + 1;
                    continue;
                }
                ...
                MultipartEvent event;
                event.kind = MultipartEventKind::PartEnd;
                event.partIndex = part;
            ...
            if ((initial && phase != Phase::Initial))
                continue;
            break;
        }
        if (eof)
        {
            FailWith(ErrorCode::InvalidFormat);
            return Fail<std::shared_ptr<const MultipartEvent>>(error);
        }
        return Fail<std::shared_ptr<const MultipartEvent>>(ErrorCode::WouldBlock);
    }
};
MultipartParser::MultipartParser(std::unique_ptr<State> state) noexcept
    : mState(std::move(state))
{
}
MultipartParser::~MultipartParser() = default;
Core::Result<std::unique_ptr<MultipartParser>> MultipartParser::Create(
    std::string_view contentType, const MultipartOptions& options)
{
    ...
    if (!options.maxParts || !options.maxFiles || !options.maxHeaders || !options.maxHeaderBytes ||
        !options.maxPartBytes || !options.maxFileBytes || !options.maxTotalBytes ||
        !options.maxEventBytes || options.maxEventBytes > options.maxRetainedBytes ||
        options.maxHeaderBytes > ((std::numeric_limits<std::size_t>::max)() - 80) / 2 ||
        options.maxRetainedBytes < options.maxHeaderBytes * 2 + 80)
        return Fail<std::unique_ptr<MultipartParser>>(ErrorCode::InvalidArgument);
    if (contentType.size() > options.maxHeaderBytes)
        return Fail<std::unique_ptr<MultipartParser>>(ErrorCode::TooLarge);
    try
    {
        auto parsed = Parameters(contentType);
        if (!parsed.IsOk())
            return Result::FromStatus(std::move(parsed).TakeStatus());
        if (parsed.Value().main != "multipart/form-data")
            return Fail<std::unique_ptr<MultipartParser>>(ErrorCode::InvalidFormat);
        std::string boundary;
        for (const auto& [name, value] : parsed.Value().parameters)
            if (name == "boundary")
                boundary = value;
        if (!Boundary(boundary))
            return Fail<std::unique_ptr<MultipartParser>>(ErrorCode::InvalidFormat);
        return Result::FromValue(std::unique_ptr<MultipartParser>(
            new MultipartParser(std::make_unique<State>(std::move(boundary), options))));
    }
    catch (...)
    {
...
{
    auto& state = *mState;
    if (state.error != ErrorCode::Ok)
        return Fail<std::size_t>(state.error);
    if (state.eof)
        return Fail<std::size_t>(ErrorCode::Closed);
    if (bytes.empty())
        return Core::Result<std::size_t>::FromValue(0);
    if (bytes.size() > state.options.maxTotalBytes - state.total)
    {
        state.FailWith(ErrorCode::TooLarge);
        return Fail<std::size_t>(state.error);
    }
    ...
    if (!count)
        return Fail<std::size_t>(ErrorCode::WouldBlock);
    try
    {
        state.buffer.append(reinterpret_cast<const char*>(bytes.data()), count);
        state.total += count;
    }
    catch (...)
    {
        state.FailWith(ErrorCode::PlatformError);
        return Core::Result<std::size_t>::FromStatus(Status::AllocationFailure());
    }
...
Core::Result<std::shared_ptr<const MultipartEvent>> MultipartParser::Read()
{
    ...
    {
        mState->FailWith(ErrorCode::PlatformError);
        return Core::Result<std::shared_ptr<const MultipartEvent>>::FromStatus(
            Status::AllocationFailure());
    }
}
Core::Status MultipartParser::Finish() noexcept
{
    if (mState->error != ErrorCode::Ok)
        return Status::FailWithoutMessage(mState->error);
    mState->eof = true;
    return Status::Ok();
}
...
std::size_t MultipartParser::RetainedBytes() const noexcept
{
