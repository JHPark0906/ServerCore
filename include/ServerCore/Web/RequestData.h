...
SERVERCORE_API Core::Result<std::string> PercentDecodeComponent(std::string_view input,
    bool plusAsSpace = false, std::size_t maxBytes = 64 * 1024);
...
SERVERCORE_API Core::Result<std::string> PercentEncodeComponent(std::string_view input,
    bool formMode = false, std::size_t maxBytes = 64 * 1024);
...
SERVERCORE_API Core::Result<FormFields> ParseQueryParameters(std::string_view requestTarget,
    const FieldLimits& limits = {});
SERVERCORE_API Core::Result<FormFields> ParseFormUrlEncoded(std::string_view body,
    const FieldLimits& limits = {});
...
SERVERCORE_API Core::Result<FormFields> ParseCookieHeader(std::string_view header,
    const FieldLimits& limits = {});
enum class CookieSameSite { Unspecified, Lax, Strict, None };
...
SERVERCORE_API Core::Result<std::string> SerializeSetCookie(std::string_view name,
    std::string_view value, const CookieOptions& options = {});
}
