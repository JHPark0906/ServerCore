...
public:
    ...
    SERVERCORE_API static Result<IpAddress> FromBytes(
        IpFamily family, std::span<const std::uint8_t> bytes, std::uint32_t scopeId = 0) noexcept;
...
struct IpEndpoint
{
    IpAddress address;
    std::uint16_t port = 0;
    SERVERCORE_API static Result<IpEndpoint> Parse(
