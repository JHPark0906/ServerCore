 ...
 * Concurrent operations are safe; destroy must not race calls on its handle. */
    SC_API sc_status sc_channel_create(
        const sc_channel_options* options, sc_channel** out) SC_NOEXCEPT;
    SC_API sc_status sc_channel_retain(const sc_channel* channel, sc_channel** out) SC_NOEXCEPT;
    SC_API void sc_channel_destroy(sc_channel* channel) SC_NOEXCEPT;
    SC_API void sc_channel_close(sc_channel* channel) SC_NOEXCEPT;
    SC_API sc_status sc_channel_try_send(sc_channel* channel, sc_bytes value) SC_NOEXCEPT;
    SC_API sc_status sc_channel_try_receive(
        sc_channel* channel, sc_channel_value** out) SC_NOEXCEPT;
    SC_API size_t sc_channel_size(const sc_channel* channel) SC_NOEXCEPT;
    SC_API size_t sc_channel_retained_bytes(const sc_channel* channel) SC_NOEXCEPT;
 ...
 * checking try_send/try_receive. Destroy subscription to cancel/rearm safely. */
    SC_API sc_status sc_channel_subscribe_read(sc_channel* channel, sc_notifier* notifier,
        uint64_t key, sc_subscription** out) SC_NOEXCEPT;
    SC_API sc_status sc_channel_subscribe_write(sc_channel* channel, size_t required_bytes,
        sc_notifier* notifier, uint64_t key, sc_subscription** out) SC_NOEXCEPT;
 ...
 * share a state; further subscriptions return TooLarge (see Types.h). */
    SC_API sc_status sc_latest_value_create(
        size_t max_retained_bytes, sc_latest_value** out) SC_NOEXCEPT;
    SC_API sc_status sc_latest_value_retain(
        const sc_latest_value* value, sc_latest_value** out) SC_NOEXCEPT;
    SC_API void sc_latest_value_destroy(sc_latest_value* value) SC_NOEXCEPT;
    SC_API void sc_latest_value_close(sc_latest_value* value) SC_NOEXCEPT;
    SC_API sc_status sc_latest_value_publish(sc_latest_value* value, sc_bytes bytes) SC_NOEXCEPT;
    SC_API sc_status sc_latest_value_read_after(
        sc_latest_value* value, uint64_t version, sc_channel_value** out) SC_NOEXCEPT;
    SC_API size_t sc_latest_value_retained_bytes(const sc_latest_value* value) SC_NOEXCEPT;
 ...
 * publication version. Destroy exactly once; views must not outlive the owner. */
    SC_API sc_status sc_channel_value_view(
        const sc_channel_value* value, sc_bytes* out) SC_NOEXCEPT;
    SC_API uint64_t sc_channel_value_version(const sc_channel_value* value) SC_NOEXCEPT;
    SC_API void sc_channel_value_destroy(sc_channel_value* value) SC_NOEXCEPT;
