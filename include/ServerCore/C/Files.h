...
extern "C"
{
    ...
    SC_API sc_status sc_atomic_file_options_init(sc_atomic_file_options*, size_t size);
    SC_API int sc_file_supports_directory_sync(void);
 ...
 * unsupported modes fail before creating a file; no durability downgrade. */
    SC_API sc_status sc_atomic_file_create(
        sc_bytes target, const sc_atomic_file_options*, sc_atomic_file** out);
    SC_API sc_status sc_atomic_file_write(sc_atomic_file*, sc_bytes data);
    SC_API sc_status sc_atomic_file_commit(sc_atomic_file*);
    /* Inspect committed even after commit errors: post-rename sync may fail. */
    SC_API int sc_atomic_file_committed(const sc_atomic_file*);
    SC_API uint64_t sc_atomic_file_written(const sc_atomic_file*);
    SC_API sc_status sc_atomic_file_cancel(sc_atomic_file*);
    SC_API void sc_atomic_file_destroy(sc_atomic_file*);
#ifdef __cplusplus
}
#endif
#endif
