...
extern "C"
{
    ...
    SC_API sc_status sc_http_policy_options_init(sc_http_policy_options*, size_t);
    SC_API sc_status sc_cors_options_init(sc_cors_options*, size_t);
    SC_API sc_status sc_proxy_options_init(sc_proxy_options*, size_t);
    SC_API sc_status sc_trusted_proxy_create(const sc_proxy_options*, sc_trusted_proxy**);
    SC_API void sc_trusted_proxy_destroy(sc_trusted_proxy*);
    SC_API sc_status sc_trusted_proxy_resolve(
        const sc_trusted_proxy*, const sc_ip_endpoint*, const sc_header*, size_t, sc_proxy_peer**);
    SC_API sc_status sc_proxy_peer_get(const sc_proxy_peer*, sc_proxy_peer_view*);
    SC_API void sc_proxy_peer_destroy(sc_proxy_peer*);
    SC_API sc_status sc_http_policy_create(const sc_http_policy_options*, sc_http_policy**);
 ...
 * inputs, including proxy configuration. Prefixes use decoded whole segments. */
    SC_API sc_status sc_http_policy_add_headers(
        sc_http_policy*, sc_bytes prefix, const sc_header*, size_t);
    SC_API sc_status sc_http_policy_add_cors(
        sc_http_policy*, sc_bytes prefix, const sc_cors_options*);
    SC_API sc_status sc_http_policy_add_request_id(sc_http_policy*, sc_bytes prefix,
        sc_bytes header, uint32_t accept_incoming, size_t max_bytes);
    SC_API sc_status sc_http_policy_add_proxy(
        sc_http_policy*, sc_bytes prefix, const sc_trusted_proxy*);
    SC_API sc_status sc_http_policy_evaluate(sc_http_policy*, const sc_request_view*,
        const sc_ip_endpoint* peer, uint32_t websocket_upgrade, sc_http_policy_result**);
    /* Works on HTTP and policy events; result owns all storage and outlives event. */
    SC_API sc_status sc_http_policy_evaluate_event(
        sc_http_policy*, const sc_web_event*, sc_http_policy_result**);
    SC_API sc_status sc_http_policy_result_get(const sc_http_policy_result*, sc_http_policy_view*);
    SC_API sc_status sc_http_policy_result_apply(
        const sc_http_policy_result*, sc_request_decision*);
    SC_API void sc_http_policy_result_destroy(sc_http_policy_result*);
    SC_API void sc_http_policy_destroy(sc_http_policy*);
 ...
 * diagnostic messages. Results can be used with response_complete or rejection. */
    SC_API sc_status sc_http_json_response(
        sc_bytes json, uint32_t status, size_t max_bytes, sc_http_policy_result**);
    SC_API sc_status sc_http_error_response(
        sc_status error, sc_bytes request_id, uint32_t status, sc_http_policy_result**);
#ifdef __cplusplus
}
#endif
#endif
