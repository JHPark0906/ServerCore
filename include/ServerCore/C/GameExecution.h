...
extern "C"
{
    ...
    /* The library fills abi_version and struct_size; read a field only when
 ...
 * are retained through capture cleanup. Existing runtime shutdown rules apply. */
    SC_API sc_status sc_tick_options_init(sc_tick_options*, size_t) SC_NOEXCEPT;
    SC_API sc_status sc_tick_start(
        sc_timer_scheduler*, const sc_tick_options*, sc_tick_work, sc_tick**) SC_NOEXCEPT;
    SC_API int sc_tick_token_requested(const sc_tick_token*) SC_NOEXCEPT;
    SC_API void sc_tick_cancel(sc_tick*) SC_NOEXCEPT;
    SC_API sc_status sc_tick_result(const sc_tick*) SC_NOEXCEPT;
    SC_API int sc_tick_finished(const sc_tick*) SC_NOEXCEPT;
    SC_API sc_status sc_tick_wait(const sc_tick*, uint32_t timeout_ms) SC_NOEXCEPT;
    SC_API sc_status sc_tick_get_metrics(const sc_tick*, sc_tick_metrics*) SC_NOEXCEPT;
    SC_API sc_status sc_tick_subscribe(
        const sc_tick*, sc_notifier*, uint64_t, sc_subscription**) SC_NOEXCEPT;
    SC_API void sc_tick_destroy(sc_tick*) SC_NOEXCEPT;
    ...
    typedef struct sc_outbound_metrics
    {
        ...
        /* Wake timers postponed because the scheduler was full; expiry and
         * continued pumping were delayed by that many rearms. */
        uint64_t deferred_wakes;
    } sc_outbound_metrics;
    SC_API sc_status sc_outbound_options_init(sc_outbound_options*, size_t) SC_NOEXCEPT;
    SC_API sc_status sc_outbound_item_options_init(sc_outbound_item_options*, size_t) SC_NOEXCEPT;
 ...
 * One transport capacity wait is exclusively used by this queue while blocked. */
    SC_API sc_status sc_outbound_create(sc_timer_scheduler*, sc_tcp_connection*,
        const sc_outbound_options*, sc_outbound_queue**) SC_NOEXCEPT;
    SC_API sc_status sc_outbound_enqueue(
        sc_outbound_queue*, sc_bytes, const sc_outbound_item_options*) SC_NOEXCEPT;
 ...
 * Invalid slot pointers/counts fail before any recipient admission. Any non-OK
 * return means no recipient admitted the item, so a retry cannot duplicate it
 * (CAbi.OutboundOwnershipAndBatch injects allocation failures). */
    SC_API sc_status sc_outbound_batch(sc_outbound_queue* const*, size_t, sc_bytes,
        const sc_outbound_item_options*, sc_status* results, size_t result_count) SC_NOEXCEPT;
    SC_API void sc_outbound_begin_drain(sc_outbound_queue*) SC_NOEXCEPT;
    SC_API void sc_outbound_close(sc_outbound_queue*) SC_NOEXCEPT;
    SC_API int sc_outbound_finished(const sc_outbound_queue*) SC_NOEXCEPT;
    SC_API sc_status sc_outbound_result(const sc_outbound_queue*) SC_NOEXCEPT;
    SC_API sc_status sc_outbound_get_metrics(
        const sc_outbound_queue*, sc_outbound_metrics*) SC_NOEXCEPT;
    SC_API sc_status sc_outbound_subscribe(
        const sc_outbound_queue*, sc_notifier*, uint64_t, sc_subscription**) SC_NOEXCEPT;
    SC_API void sc_outbound_destroy(sc_outbound_queue*) SC_NOEXCEPT;
#ifdef __cplusplus
}
#endif
#endif
