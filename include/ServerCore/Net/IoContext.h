...
/// 순서를 어겨 이것이 연결보다 먼저 소멸해도 연결의 공개 함수는 해제된 메모리에 닿지 않고 닫힘이나
/// 무해한 결과를 돌려준다. 이것을 고정하는 것은 Transport.ConnectionOutlivesIoContext다. 다만 그때
/// 끝나지 못한 I/O와 끊김 통지는 처리할 스레드가 없으므로 약속하지 않는다.
///
/// 약속하지 않는 것:
