...
/// <remarks>
/// 호출 계약:
/// - OnBytesReceived는 I/O 스레드에서 불린다. OnDisconnected는 마지막 작업을 처리한 I/O
...
/// 부분 전송된 큐 맨 앞의 보낸 prefix도 송신 작업이 그 vector를 완전히 놓을 때까지 이 상한에
...
/// 소유하지 않는다: 관찰자. 약한 참조로만 잡는다. IoContext도, 수락기도 소유하지 않는다.
///
/// 소유권과 수명:
/// 항상 std::shared_ptr로만 다룬다. 진행 중인 I/O 작업이 자기 몫의 참조를 들고 있으므로,
...
/// 플랫폼 구현이 연결별 종료 상태와 통지 상태를 동기화한다. 여러 I/O 스레드가 동시에
/// 끊김을 발견하더라도 관찰자를 부를 수 있는 경로는
/// 하나로 제한한다. 관찰자가 걸린 연결은 수신 콜백의 종료를 확인한 뒤 종료 통지 경로를
/// 지난다. 구체적인 상태 보호 수단은 플랫폼 구현 안에 둔다.
///
/// 통지는 진행 중인 I/O 작업과 수신 콜백이 전부 끝난 뒤에 간다. 그래서 OnDisconnected를 받은 뒤에는
...
public:
    ...
    /// 닫기를 택해 남은 바이트를 버린다. 다만 진행 중인 송신 작업이 큐의 버퍼를 사용하면 그
    /// 작업이 끝나기 전에는 버퍼를 해제할 수 없다. 그 동안 QueuedSendBytes()가 잠시 남을 수
    /// 있지만, OnDisconnected 통지보다 먼저 남은 큐를 모두 버린다.
    ///
    /// 이 호출은 진행 중인 I/O 작업과 콜백의 종료를 기다리지 않는다. 이 연결이 쓰는 IoContext를
    ...
    /// 모두 운영체제의 송신 처리까지 진행한 뒤 보내기 쪽을 정상 종료한다. 그 뒤 Send()는 비어 있는
