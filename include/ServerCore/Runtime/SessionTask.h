...
SERVERCORE_API Core::Result<TaskHandle> SubmitSessionTask(TaskExecutor& executor, JobRunner::Lease runner,
    ...
    const SessionTaskOptions& options = {})
{
    ...
    return SubmitSessionTask(executor, std::move(runner), session,
        std::move(task), std::move(apply), options);
}
}
