...
SERVERCORE_API Core::Result<std::string> RenderPrometheus(
    const TaskExecutorMetricsSnapshot& snapshot);
SERVERCORE_API Core::Result<std::string> RenderPrometheus(
    const HttpServerMetricsSnapshot& snapshot);
SERVERCORE_API Core::Result<std::string> RenderPrometheus(const JobRunnerMetricsSnapshot& snapshot);
SERVERCORE_API Core::Result<std::string> RenderPrometheus(
    const Runtime::ServerMetricsSnapshot& snapshot);
}
