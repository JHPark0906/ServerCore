...
[[nodiscard]] SERVERCORE_API ObservationSnapshot Observe(
    const TaskExecutorMetricsSnapshot&) noexcept;
[[nodiscard]] SERVERCORE_API ObservationSnapshot Observe(const LoggerMetricsSnapshot&) noexcept;
[[nodiscard]] SERVERCORE_API ObservationSnapshot Observe(
    const Runtime::ServerMetricsSnapshot&) noexcept;
...
[[nodiscard]] SERVERCORE_API ObservationSnapshot Observe(
    const Runtime::DatagramTransport&) noexcept;
// Preserves ServerHost::SnapshotMetrics's JobRunner-context requirement; a stopped Host is
