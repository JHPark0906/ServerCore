...
void StreamingResponseFraming()
{
    ...
    if (chunked.IsOk())
    {
        ExpectTrue(chunked.Value().mode == Detail::ResponseBodyMode::Chunked, "unknown length selects chunked framing");
        ExpectTrue(chunked.Value().wire.find("Transfer-Encoding: chunked\r\n") != std::string::npos &&
            chunked.Value().wire.find("Content-Length:") == std::string::npos, "chunked response has one framing mechanism");
    ...
    ExpectTrue(largeHead.IsOk(), "HEAD metadata supports a body larger than memory and local send limits");
    if (largeHead.IsOk())
    {
        ExpectTrue(largeHead.Value().mode == Detail::ResponseBodyMode::None, "HEAD emits no content");
        ExpectEqual(*head.contentLength, largeHead.Value().length, "HEAD keeps full 64-bit representation length");
        ExpectTrue(largeHead.Value().wire.find("Content-Length: 9663676416\r\n") != std::string::npos,
    ...
    if (unknownHead.IsOk())
        ExpectTrue(unknownHead.Value().mode == Detail::ResponseBodyMode::None &&
            ...
            unknownHead.Value().wire.find("Content-Length:") == std::string::npos, "unknown HEAD has no body framing bytes");
    for (const auto status : {200u, 204u, 205u, 304u})
    {
        for (const bool isHead : {false, true})
        {
            ...
            auto prepared = Detail::PrepareResponseHead(status, buffered.headers, std::uint64_t{6}, isHead, true, 1024);
            std::string legacy;
            ExpectTrue(prepared.IsOk() && Detail::SerializeResponse(buffered, isHead, true, 1024, 6, legacy),
                "buffered and streaming response heads share status validation");
            if (!prepared.IsOk()) continue;
            auto expected = prepared.Value().wire;
            if (prepared.Value().mode != Detail::ResponseBodyMode::None) expected += buffered.body;
            ExpectEqual(expected, legacy, "legacy serializer shares identical framing policy");
            if (status == 204 || status == 304)
                ExpectTrue(legacy.find("Content-Length:") == std::string::npos && legacy.find("Transfer-Encoding:") == std::string::npos,
                    "204 and 304 omit body framing");
            if (status == 205)
                ExpectTrue(legacy.find("Content-Length: 0\r\n") != std::string::npos && legacy.ends_with("\r\n\r\n"),
                    "205 is explicitly empty on a persistent HTTP connection");
        }
    }
    std::string unchanged = "unchanged";
    ExpectTrue(!Detail::SerializeResponse(Web::HttpResponse{200, {}, "abcdef"}, false, false, 1024, 5, unchanged),
        "buffered response limit remains enforced independently of streaming support");
    ExpectEqual(std::string("unchanged"), unchanged, "oversized buffered response is rejected before output");
}
...
void StreamingResponseRejectsInvalidHeaders()
{
    const std::array<std::pair<std::string_view, std::string_view>, 8> invalid{{
        ...
        {"Trailer", "X-Trailer"}, {"bad name", "value"}, {"X-Test", "value\r\nInjected: yes"},
        {"Upgrade", "bad protocol"}, {"X-Test", "\x7f"}
    }};
    for (const auto& [name, value] : invalid)
    {
        Web::HttpResponseHead head;
        head.headers.emplace_back(name, value);
        ExpectCode(ErrorCode::InvalidArgument, Detail::PrepareResponseHead(head, false, 1024).GetStatus(),
    ...
    head.status = 600;
    ExpectCode(ErrorCode::InvalidArgument, Detail::PrepareResponseHead(head, false, 1024).GetStatus(), "status is bounded");
    head.status = 426;
    ExpectCode(ErrorCode::InvalidArgument, Detail::PrepareResponseHead(head, false, 1024).GetStatus(), "426 requires Upgrade");
    ...
    if (upgrade.IsOk())
        ExpectTrue(upgrade.Value().wire.find("Connection: keep-alive, Upgrade\r\n") != std::string::npos,
            "framework adds the required Upgrade connection option");
    head.status = 200;
    head.headers = {{"Date", "Sun, 06 Nov 1994 08:49:37 GMT"}, {"date", "duplicate"}};
    ExpectCode(ErrorCode::InvalidArgument, Detail::PrepareResponseHead(head, false, 1024).GetStatus(), "duplicate Date is invalid");
    ...
    if (valid.IsOk())
    {
        ...
        ExpectTrue(Detail::PrepareResponseHead(head, false, size).IsOk(), "exact header byte limit is accepted");
        ExpectCode(ErrorCode::TooLarge, Detail::PrepareResponseHead(head, false, size - 1).GetStatus(),
...
void HttpChunkEncoding()
{
    ...
    if (result.IsOk()) ExpectEqual(std::string("3\r\n") + binary + "\r\n", result.Value(), "chunk framing preserves binary bytes");
    ...
    if (hexadecimal.IsOk()) ExpectEqual(std::string("10\r\n") + sixteen + "\r\n", hexadecimal.Value(), "chunk length uses hexadecimal");
    ExpectCode(ErrorCode::TooLarge, Detail::EncodeChunk(Bytes(binary), 2).GetStatus(), "oversized chunk is rejected atomically");
    ExpectCode(ErrorCode::InvalidArgument, Detail::EncodeChunk({}, 3).GetStatus(), "empty write cannot accidentally terminate a stream");
    ExpectEqual(std::string("0\r\n\r\n"), std::string(Detail::FinalChunk), "Finish has a distinct complete final chunk");
}
...
public:
    ...
    {
        ++writes;
        if (writeCode != ErrorCode::Ok) return Status::FailWithoutMessage(writeCode);
    ...
    void Abort() noexcept override { ++aborts; (void)cancellation.request_stop(); }
    std::stop_token GetCancellationToken() const noexcept override { return cancellation.get_token(); }
...
void SseEncoding()
{
    ...
    const std::string expected = "event: tick\nid: \nretry: 0\ndata: a\ndata: b\ndata: c\ndata: \n\n";
    auto encoded = Web::EncodeSseEvent(event, expected.size());
    ExpectTrue(encoded.IsOk(), "SSE event encodes at exact byte limit");
    if (encoded.IsOk()) ExpectEqual(expected, encoded.Value(), "SSE normalizes lines and preserves explicit empty id and trailing line");
    ExpectCode(ErrorCode::TooLarge, Web::EncodeSseEvent(event, expected.size() - 1).GetStatus(), "SSE limit includes all field prefixes and terminator");
    event = {};
    auto empty = Web::EncodeSseEvent(event, 8);
    ExpectTrue(empty.IsOk(), "empty data event is representable");
    if (empty.IsOk()) ExpectEqual(std::string("data: \n\n"), empty.Value(), "empty event has a data field and dispatching blank line");
    event.data = "first\nid: injected";
    auto injection = Web::EncodeSseEvent(event, 128);
    ExpectTrue(injection.IsOk(), "multiline data accepts field-like text");
    if (injection.IsOk()) ExpectEqual(std::string("data: first\ndata: id: injected\n\n"), injection.Value(), "data cannot inject metadata fields");
    event.data = "\xC0\xAF";
    ExpectCode(ErrorCode::InvalidArgument, Web::EncodeSseEvent(event, 128).GetStatus(), "SSE rejects invalid UTF-8");
    event.data = "valid";
    event.event = "tick\nid: injected";
    ExpectCode(ErrorCode::InvalidArgument, Web::EncodeSseEvent(event, 128).GetStatus(), "event name cannot inject lines");
    event.event.clear();
    event.id = std::string("a\0b", 3);
    ExpectCode(ErrorCode::InvalidArgument, Web::EncodeSseEvent(event, 128).GetStatus(), "SSE id cannot contain NUL");
    event.id = "id\rnext";
    ExpectCode(ErrorCode::InvalidArgument, Web::EncodeSseEvent(event, 128).GetStatus(), "SSE id cannot inject lines");
    event.id.reset();
    RecordingWriter writer;
    writer.writeCode = ErrorCode::WouldBlock;
    ExpectCode(ErrorCode::WouldBlock, Web::WriteSseEvent(writer, event), "SSE forwards bounded writer backpressure");
    ...
    ExpectEqual(std::string("data: valid\n\n"), writer.payload, "successful retry emits exactly one complete event");
}
...
void FileTaskQueuedCancellation()
{
    ...
    bool entered = false;
    bool release = false;
    auto blocking = executor.Submit([&](std::stop_token) {
    ...
    {
        std::unique_lock guard(mutex);
        ExpectTrue(wake.wait_for(guard, 2s, [&entered] { return entered; }), "worker is occupied before file admission");
    }
    ...
    if (file.IsOk())
    {
        ...
        ExpectCode(ErrorCode::Cancelled, file.Value().WaitUntil(std::chrono::steady_clock::now() + 2s),
            "queued file cancellation finishes without reading its path");
        ExpectEqual(1u, writer->aborts.load(), "discarded queued file task aborts its HTTP response exactly once");
    }
    ...
    ExpectCode(ErrorCode::TooLarge, Web::SendFile(executor, context, "unused", std::move(hugeMetadata)).GetStatus(),
        "queued file metadata is included in executor retained admission");
    ExpectEqual(beforeMetadataRejection, writer->aborts.load(), "metadata rejection leaves the response untouched");
    ...
    ExpectTrue(compact.IsOk(), "spare caller header capacity is removed before bounded file admission");
    if (compact.IsOk())
    {
        (void)compact.Value().RequestCancel();
        ExpectCode(ErrorCode::Cancelled, compact.Value().WaitUntil(std::chrono::steady_clock::now() + 2s),
    ...
    ExpectCode(ErrorCode::Closed, Web::SendFile(executor, context, "file-that-must-not-be-opened").GetStatus(),
...
{
public:
    explicit ResponseTestConnection(std::shared_ptr<ServerCore::Net::SendBudget> budget) : queue(std::move(budget)) {}
    Status Send(std::span<const std::byte> bytes) override
    {
        if (!open) return Status::FailWithoutMessage(ErrorCode::Closed);
        if (auto callback = std::exchange(onSend, {})) callback();
        return queue.Enqueue(bytes);
    }
    void Close() override { open = false; queue.CloseCapacityWaits(); queue.Budget()->Notify(); }
...
void ResponseWaitLifetimeAndReentrancy()
{
    ...
    auto first = std::make_shared<Detail::HttpResponseState>(sender, options, false, false, false, nullptr, nullptr);
    Web::HttpResponseHead head;
    head.contentLength = 0;
    ...
    ExpectTrue(blocker.Enqueue(Bytes(payload)).IsOk(), "another queue consumes all shared capacity");
    unsigned calls = 0;
    auto wait = first->WaitForWriteCapacity(1, [&](Status result) {
        ++calls;
        ExpectCode(ErrorCode::Cancelled, result, "successful Finish retires its accepted capacity wait");
        ExpectCode(ErrorCode::Closed, first->Finish(), "terminal wait callback can reenter the writer without a lock");
    });
    ExpectTrue(wait.IsOk() && wait.Value().IsPending(), "first response owns a pending capacity wait");
    ...
    ExpectTrue(!first->GetCancellationToken().stop_requested(), "successful response does not cancel application work");
    auto second = std::make_shared<Detail::HttpResponseState>(sender, options, false, false, false, nullptr, nullptr);
    auto successor = second->WaitForWriteCapacity(0, [](Status) {});
    ExpectTrue(successor.IsOk(), "next response can wait even while the old subscription remains owned");
    if (successor.IsOk()) successor.Value().Reset();
    blocker.Clear();
    budget->Notify();
    first->Abort();
    ...
    connection->onSend = [&] {
        ExpectCode(ErrorCode::WouldBlock, second->Write(Bytes("b")), "reentrant write cannot overlap an active admission");
        auto retry = second->WaitForWriteCapacity(1, [&](Status) { ++inlineCalls; });
        ExpectCode(ErrorCode::WouldBlock, retry.GetStatus(), "capacity registration cannot recursively retry an unfinished admission");
    };
    ExpectTrue(second->Write(Bytes("a")).IsOk(), "outer send completes after bounded reentrant calls");
    ExpectEqual(0u, inlineCalls, "rejected concurrent wait never invokes its callback");
    ExpectTrue(second->Write(Bytes("b")).IsOk() && second->Finish().IsOk(), "serialized producer resumes after admission returns");
}

const ServerCoreTest::CheckRegistration framing("Web.StreamingResponseFraming", StreamingResponseFraming);
const ServerCoreTest::CheckRegistration validation("Web.StreamingResponseRejectsInvalidHeaders", StreamingResponseRejectsInvalidHeaders);
...
const ServerCoreTest::CheckRegistration file("Web.FileTaskQueuedCancellation", FileTaskQueuedCancellation);
const ServerCoreTest::CheckRegistration lifetime("Web.ResponseWaitLifetimeAndReentrancy", ResponseWaitLifetimeAndReentrancy);
}
