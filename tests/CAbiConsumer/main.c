...
static sc_status run_task(void* context, const sc_runtime_token* token)
{
    ...
    if (sc_runtime_token_requested(token)) return SC_CANCELLED;
    *result = 23;
    return SC_OK;
}
...
int main(void)
{
    ...
    sc_tcp_server* listener = NULL;
    {
        ...
        if (sc_binary_writer_create(64, SC_BIG_ENDIAN, &binary) != SC_OK) return 14;
        if (sc_binary_write_signed(binary, -23, 2) != SC_OK) return 15;
        sc_binary_writer_destroy(binary);
        if (sc_atomic_file_options_init(&file_options, sizeof(file_options)) != SC_OK) return 16;
        if ((sc_capabilities() & (SC_CAP_GAME_EXECUTION | SC_CAP_BINARY_IO | SC_CAP_DATAGRAM |
            SC_CAP_ATOMIC_FILE | SC_CAP_OPERATIONS)) != (SC_CAP_GAME_EXECUTION | SC_CAP_BINARY_IO |
            SC_CAP_DATAGRAM | SC_CAP_ATOMIC_FILE | SC_CAP_OPERATIONS)) return 17;
    }
    {
        ...
        sc_request_limit_decision decision = {SC_ABI_VERSION, sizeof(sc_request_limit_decision), 0, 0};
        sc_bytes key = {(const uint8_t*)"consumer", 8};
        sc_http_policy_result* json = NULL;
        sc_bytes input = {(const uint8_t*)"{}", 2};
        if (sc_ip_endpoint_parse(address, 80, &endpoint) != SC_OK || endpoint.family != SC_IP_V6) return 11;
        if (sc_request_limiter_options_init(&options, sizeof(options)) != SC_OK ||
            sc_request_limiter_create(&options, &limiter) != SC_OK ||
            sc_request_limiter_acquire(limiter, key, 1, &decision, &permit) != SC_OK || !permit) return 12;
        sc_request_limiter_destroy(limiter);
        sc_request_permit_destroy(permit);
        if (sc_http_json_response(input, 200, 1024, &json) != SC_OK) return 13;
    ...
    sc_tcp_server_destroy(listener);
    {
        ...
        if (sc_channel_create(&options, &channel) != SC_OK ||
            ...
            sc_channel_try_receive(channel, &value) != SC_OK) return 6;
        sc_channel_destroy(channel);
        if (sc_channel_value_view(value, &output) != SC_OK || output.len != 5 ||
            memcmp(output.data, input.data, input.len) != 0) return 7;
    ...
    }
    {
        ...
        if (sc_executor_options_init(&options, sizeof(options)) != SC_OK ||
            ...
            sc_executor_submit(executor, work, &task_options, &task) != SC_OK) return 8;
        if (sc_runtime_task_wait(task, 5000) != SC_OK || result != 23 ||
            sc_executor_stop(executor) != SC_OK) return 9;
        sc_runtime_task_destroy(task);
        sc_executor_destroy(executor);
    }
    if (sc_runtime_shutdown(5000) != SC_OK) return 10;
    return 0;
}
