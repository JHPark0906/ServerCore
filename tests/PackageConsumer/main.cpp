...
int main()
{
    ...
    if (!common.IsOk())
        return 1;
    auto policies =
        ServerCore::Web::HttpPolicyPipeline::Create({ { "/api", std::move(common.Value()) } });
    if (!policies.IsOk() || !policies.Value().Evaluate({ "GET", "/api/item", {}, {} }).IsOk())
        return 1;
    if (!ServerCore::Web::ErrorResponse(ServerCore::Core::ErrorCode::NotFound).IsOk())
        return 1;
    ...
    if (!result.IsOk() || *result.Value() != 23)
        return 1;
    ServerCore::Runtime::TaskExecutor executor;
    if (!executor.Start({ 1, 2, 1024 }).IsOk())
        return 1;
    if (!ServerCore::Observability::Observe(executor).Has(ServerCore::Observability::PendingWork))
        return 1;
    ...
    if (!task.IsOk() || !task.Value().Wait().IsOk())
        return 1;
    ServerCore::Runtime::TimerScheduler scheduler;
    if (!scheduler.Start(executor).IsOk())
        return 1;
    auto timer = scheduler.Schedule([](std::stop_token) { return ServerCore::Core::Status::Ok(); });
    if (!timer.IsOk() || !timer.Value().Wait().IsOk() || !scheduler.Stop().IsOk())
        return 1;
    ServerCore::Runtime::TaskGroup group;
    if (!group.Start(executor).IsOk())
        return 1;
    auto child = group.Submit([](std::stop_token) { return ServerCore::Core::Status::Ok(); });
    if (!child.IsOk() || !group.Wait().IsOk() || !group.TakeCompletions().IsOk() ||
        !group.Stop().IsOk())
        return 1;
    if (!executor.Stop().IsOk())
        return 1;
    ServerCore::Runtime::KeyedExecutor keyed;
    if (!keyed.Start().IsOk())
        return 1;
    auto keyedTask =
        keyed.Submit(7, [](std::stop_token) { return ServerCore::Core::Status::Ok(); });
    if (!keyedTask.IsOk() || !keyedTask.Value().Wait().IsOk() || !keyed.Stop().IsOk())
        return 1;
    auto channel = ServerCore::Runtime::BoundedChannel<int>::Create();
    if (!channel.IsOk() ||
        !channel.Value().TrySend(std::make_shared<const int>(23), sizeof(int)).IsOk())
        return 1;
    auto received = channel.Value().TryReceive();
    if (!received.IsOk() || *received.Value() != 23)
        return 1;
    ServerCore::Net::SendCapacitySubscription subscription;
    if (subscription.IsPending() || subscription.Cancel() ||
        ServerCore::Net::GetConnectionFlowControl({}))
        return 1;
    ...
    if (!route.IsOk() || http.IsRunning())
        return 1;
    if (!http.RegisterWebSocket("/events", {}).IsOk())
        return 1;
    const ServerCore::Web::HttpRequest legacyRequest{ "GET", "/", {}, "" };
    const ServerCore::Web::WebSocketCallbacks legacyCallbacks{ {}, {}, {}, {} };
    if (!legacyRequest.PathParameter("id").empty() || legacyCallbacks.onOpenWithRequest)
        return 1;
    if (!http
            .RegisterRoutePattern("GET", "/plugins/{id}",
                [](const ServerCore::Web::HttpRequest& request)
                {
                    return ServerCore::Web::HttpResponse{ 200, {},
                        std::string(request.PathParameter("id")) };
                })
            .IsOk())
        return 1;
    if (!http.RegisterWebSocketPattern("/events/{id}", {}).IsOk())
        return 1;
    if (!http
        ...
        return 1;
    if (!http
            .RegisterAsyncRoutePattern("GET", "/files/{id}",
                [](std::shared_ptr<const ServerCore::Web::HttpRequestContext> context)
                {
                    (void)context->response->Complete(
                        { 200, {}, std::string(context->request.PathParameter("id")) });
                })
            .IsOk())
        return 1;
    auto event = ServerCore::Web::EncodeSseEvent({ "ready", "", {}, {} }, 128);
    if (!event.IsOk() || event.Value() != "data: ready\n\n" ||
        ServerCore::Web::GetWebSocketFlowControl({}))
        return 1;
    ...
    if (!runner.IsStopRequested() ||
        runner.Post([] {}).Code() != ServerCore::Core::ErrorCode::Closed)
        return 1;
    runner.RunUntilStopped();
    if (completed != 1 || runner.PendingCount() != 0)
        return 1;

    ServerCore::Net::Acceptor acceptor;
    if (!acceptor.SetSendQueueLimits({ 1024, 4096 }).IsOk())
        return 1;
