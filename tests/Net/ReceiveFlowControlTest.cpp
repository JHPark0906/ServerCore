...
public:
    ...
    bool Connect(const std::uint16_t port)
    {
        ...
        if (mSocket == ServerCoreTest::InvalidSocket)
            return false;
        if (!ServerCoreTest::SetSocketTimeouts(mSocket, 10000))
            return false;
        sockaddr_in address{};
        ...
        if (::inet_pton(AF_INET, "127.0.0.1", &address.sin_addr) != 1)
            return false;
        return ::connect(mSocket, reinterpret_cast<const sockaddr*>(&address), sizeof(address)) ==
               0;
    }
    ...
    bool ReceiveExactly(const std::span<std::byte> bytes)
    {
        ...
        {
            const int received =
                ServerCoreTest::Receive(mSocket, reinterpret_cast<char*>(bytes.data() + offset),
                    static_cast<int>(bytes.size() - offset));
            if (received <= 0)
                return false;
...
{
public:
    explicit FlowObserver(const bool cycleInCallback = false)
        : mCycleInCallback(cycleInCallback)
    {
    }
    ...
    void OnBytesReceived(const std::span<const std::byte> bytes) override
    {
        ...
        if (mCycleInCallback)
        {
            ...
            if (!flow || !flow->PauseReceive().IsOk())
                mFailure.store(true);
            if (call == 1)
            {
            ...
            if (!flow || !flow->ResumeReceive().IsOk())
                mFailure.store(true);
            if (call == 1)
            {
                std::unique_lock lock(mMutex);
        ...
        if (!std::equal(bytes.begin(), bytes.end(), original.begin()))
            mFailure.store(true);
        {
    ...
    bool CallsStayAt(const std::size_t count)
    {
        ...
        return !mChanged.wait_for(
            lock, QuietInterval, [this, count] { return mCalls.load() != count; });
    }
...
public:
        ...
        const bool resumeBeforeStart = false, const bool holdHandler = false)
    {
        ...
        mAcceptor.SetConnectionHandler(
            [this, pauseBeforeStart, resumeBeforeStart, holdHandler](
                std::shared_ptr<Net::Connection> accepted)
            {
                ...
                if (configured && pauseBeforeStart)
                    configured = control->PauseReceive().IsOk();
                if (configured && resumeBeforeStart)
                    configured = control->ResumeReceive().IsOk();
                std::unique_lock lock(mMutex);
                ...
                mChanged.notify_all();
                if (holdHandler &&
                    !mChanged.wait_for(lock, WaitLimit, [this] { return mReleaseHandler; }))
    ...
    {
        std::unique_lock lock(mMutex);
        return mChanged.wait_for(lock, WaitLimit, [this] { return connection != nullptr; }) &&
               mConfigured;
    }
    ...
    void Shutdown()
    {
        ...
        {
            connection->Close();
            ServerCoreTest::ExpectTrue(observer->WaitForDisconnect(),
                "flow-control connection drains before IoContext stops");
        }
        mIo.Stop();
        ServerCoreTest::ExpectTrue(
            !mHandlerTimedOut, "accept handoff gate was released within its deadline");
        mStopped = true;
    }
...
bool Connect(FlowServer& server, Peer& peer, const bool paused,
    const bool resumeBeforeStart = false, const bool holdHandler = false)
{
    const auto port = ServerCoreTest::FreeLoopbackTcpPort();
    ...
    ServerCoreTest::ExpectTrue(
        accepted, "accepted connection implements and configures flow control");
    return accepted;
}
...
void PauseReceiveBeforeStart()
{
    ...
    FlowServer server;
    Peer peer;
    if (!Connect(server, peer, true))
        return;
    server.FinishAccept();
    ServerCoreTest::ExpectTrue(
        server.flow->IsReceivePaused(), "accept-handler pause survives Connection::Start");
    ...
    ServerCoreTest::ExpectTrue(
        server.observer->CallsStayAt(0), "a connection started paused submits no initial receive");
    ServerCoreTest::ExpectTrue(
        server.flow->ResumeReceive().IsOk(), "resume admits buffered peer bytes");
    ServerCoreTest::ExpectTrue(
        server.flow->ResumeReceive().IsOk(), "repeated resume succeeds without duplicate I/O");
    ServerCoreTest::ExpectTrue(!server.flow->IsReceivePaused(), "resume clears paused state");
    ServerCoreTest::ExpectTrue(
        server.observer->WaitForBytes(bytes.size()), "resumed receive makes progress");
    server.Shutdown();
    ServerCoreTest::ExpectTrue(
        server.observer->Bytes() == bytes, "pre-start pause preserves every byte exactly once");
    ServerCoreTest::ExpectTrue(
        server.observer->Healthy(), "receive and disconnect callbacks remain serialized");
}
...
void ResumeReceiveBeforeStart()
{
    ...
    FlowServer server;
    Peer peer;
    if (!Connect(server, peer, true, true, true))
        return;
    const auto bytes = Payload(8193);
    ServerCoreTest::ExpectTrue(
        !server.flow->IsReceivePaused(), "resume inside the accept handler clears paused state");
    ServerCoreTest::ExpectTrue(
        peer.SendAll(bytes), "peer queues input during the held accept handler");
    ServerCoreTest::ExpectTrue(server.observer->CallsStayAt(0),
        "pre-start resume cannot deliver input before accept handoff returns");
    server.FinishAccept();
    ServerCoreTest::ExpectTrue(server.observer->WaitForBytes(bytes.size()),
        "Connection::Start begins receiving after pre-start resume");
    server.Shutdown();
    ServerCoreTest::ExpectTrue(
        server.observer->Bytes() == bytes, "pre-start resume neither loses nor duplicates input");
    ServerCoreTest::ExpectTrue(
        server.observer->Healthy(), "pre-start resume preserves callback serialization");
}
...
void ReceiveFlowControlSerializesCallbacks()
{
    ...
    Peer peer;
    if (!Connect(server, peer, false))
        return;
    server.FinishAccept();
    const auto bytes = Payload(128 * 1024 + 23);
    ServerCoreTest::ExpectTrue(
        peer.SendAll(std::span(bytes).first(1024)), "peer starts the first receive callback");
    ...
    if (!paused)
        return;
    ServerCoreTest::ExpectTrue(
        server.flow->IsReceivePaused(), "callback pause is visible to other threads");
    ServerCoreTest::ExpectTrue(peer.SendAll(std::span(bytes).subspan(1024)),
        "peer queues multiple receive buffers behind the active callback");
    ServerCoreTest::ExpectTrue(server.observer->ResumeFirstAndWait(),
        "first callback resumes while its borrowed span is live");
    ServerCoreTest::ExpectTrue(server.observer->CallsStayAt(1),
        "resume inside a callback cannot start a concurrent callback");
    server.observer->ReleaseCallbacks();
    ServerCoreTest::ExpectTrue(server.observer->WaitForBytes(bytes.size()),
        "repeated callback pause/resume consumes all buffered input");
    server.Shutdown();
    ServerCoreTest::ExpectTrue(
        server.observer->Calls() > 1, "payload exercised multiple receive callbacks");
    ServerCoreTest::ExpectTrue(server.observer->Bytes() == bytes,
        "callback pause/resume preserves payload order and contents");
    ServerCoreTest::ExpectTrue(server.observer->Healthy(),
        "no callback overlap, borrowed buffer corruption, or flow-control failure");
}
...
void SendWhileReceivePaused()
{
    ...
    if (sent)
        ServerCoreTest::ExpectTrue(
            peer.ReceiveExactly(received), "outgoing writes progress while receive remains paused");
    ServerCoreTest::ExpectTrue(received == outbound, "paused connection sends the exact payload");
    ServerCoreTest::ExpectTrue(
        server.observer->CallsStayAt(0), "write readiness never bypasses receive pause");
    ServerCoreTest::ExpectTrue(
        server.flow->ResumeReceive().IsOk(), "input can resume after writes drain");
    ServerCoreTest::ExpectTrue(server.observer->WaitForBytes(inbound.size()),
        "buffered input survives paused outgoing writes");
    server.Shutdown();
    ServerCoreTest::ExpectTrue(
        server.observer->Bytes() == inbound, "full-duplex flow control preserves inbound bytes");
    ServerCoreTest::ExpectTrue(
        server.observer->Healthy(), "paused write callbacks preserve receive serialization");
}
...
void PausedPeerEofDisconnectsOnce()
{
    ...
    for (unsigned attempt = 0; attempt < 2; ++attempt)
    {
        ...
        server.FinishAccept();
        ServerCoreTest::ExpectTrue(
            peer.ShutdownSend(), "peer queues graceful EOF while receiving is paused");
        ServerCoreTest::ExpectTrue(server.observer->DisconnectsStayAtZero(),
            "paused connection has not submitted an EOF-consuming receive");
        if (attempt == 0)
            ServerCoreTest::ExpectTrue(server.flow->ResumeReceive().IsOk(),
                "resuming a paused connection admits pending EOF");
        else
            server.connection->Close();
        ServerCoreTest::ExpectTrue(server.observer->WaitForDisconnect(),
            "resume or explicit close completes paused EOF teardown");
        ...
        server.connection->Close();
        server.Shutdown();
        ServerCoreTest::ExpectEqual(std::size_t{ 1 }, server.observer->Disconnects(),
            "paused EOF disconnects exactly once");
        ServerCoreTest::ExpectEqual(std::size_t{ 0 }, server.observer->Calls(),
            "EOF is never delivered as an empty payload callback");
        ServerCoreTest::ExpectTrue(
            server.observer->Healthy(), "paused EOF teardown does not overlap receive callbacks");
    }
}
...
void SendQueueLimitsConfiguration()
{
    ...
    FlowServer server;
    ServerCoreTest::ExpectTrue(
        server.ConfigureLimits({ 0, 16 }).Code() == Core::ErrorCode::InvalidArgument,
        "zero per-connection send capacity is invalid");
    ServerCoreTest::ExpectTrue(
        server.ConfigureLimits({ 8, 0 }).Code() == Core::ErrorCode::InvalidArgument,
        "zero shared send capacity is invalid");
    ServerCoreTest::ExpectTrue(
        server.ConfigureLimits({ 1024 * 1024 + 1, 16 }).Code() == Core::ErrorCode::InvalidArgument,
        "per-connection send capacity above 1 MiB is invalid");
    ServerCoreTest::ExpectTrue(server.ConfigureLimits({ 8, 512 * 1024 * 1024 + 1 }).Code() ==
    ...
    ServerCoreTest::ExpectTrue(server.ConfigureLimits({ 8, 16 }).IsOk(),
        "small send limits can be configured before Start");
    Peer peer;
    if (!Connect(server, peer, true))
        return;
    ...
    ServerCoreTest::ExpectTrue(
        server.connection->Send(tooLarge).Code() == Core::ErrorCode::WouldBlock,
    ...
    auto wait =
        server.flow->WaitForSendCapacity(9, [&callbacks](Core::Status) { callbacks.fetch_add(1); });
    ...
    if (sent)
        ServerCoreTest::ExpectTrue(
            peer.ReceiveExactly(received), "configured capacity still permits real TCP sends");
    ServerCoreTest::ExpectTrue(received == bytes, "accepted boundary payload arrives intact");
    server.Shutdown();
    ServerCoreTest::ExpectEqual(
        0U, callbacks.load(), "rejected capacity wait never invokes its callback");
    ServerCoreTest::ExpectEqual(std::size_t{ 0 }, server.flow->RetainedSendBytes(),
        "disconnect releases all retained bytes under custom limits");
    ServerCoreTest::ExpectTrue(server.ConfigureLimits({ 16, 32 }).IsOk(),
        "a stopped acceptor permits a new budget configuration");
}

const ServerCoreTest::CheckRegistration pauseBeforeStart{ "Transport.PauseReceiveBeforeStart",
    &PauseReceiveBeforeStart };
const ServerCoreTest::CheckRegistration resumeBeforeStart{ "Transport.ResumeReceiveBeforeStart",
    &ResumeReceiveBeforeStart };
...
const ServerCoreTest::CheckRegistration sendWhilePaused{ "Transport.SendWhileReceivePaused",
    &SendWhileReceivePaused };
const ServerCoreTest::CheckRegistration pausedEof{ "Transport.PausedPeerEofDisconnectsOnce",
    &PausedPeerEofDisconnectsOnce };
const ServerCoreTest::CheckRegistration sendQueueLimits{ "Transport.SendQueueLimitsConfiguration",
    &SendQueueLimitsConfiguration };
}
