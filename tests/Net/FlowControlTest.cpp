...
{
public:
    explicit Queue(std::shared_ptr<SendBudget> budget)
...
SendCapacitySubscription Accepted(Queue& queue, std::size_t bytes,
    std::function<void(Status)> callback, std::stop_token cancellation = {})
{
...
void SendCapacityRetainsPartialStorage()
{
    ...
    queue.Consume(7);
    ExpectEqual(
        std::size_t{ 1 }, queue.QueuedBytes(), "logical bytes decrease after partial completion");
    ExpectEqual(std::size_t{ 8 }, queue.RetainedBytes(),
        "sent prefix remains retained until full front release");
    ExpectEqual(
        std::size_t{ 8 }, budget->UsedBytes(), "shared budget also retains the sent prefix");
    budget->MarkChanged();
    budget->Notify();
    ExpectEqual(0u, calls, "shared free space cannot bypass full local retained storage");
    ExpectCode(ErrorCode::WouldBlock, queue.Send("x"),
        "partially sent allocation still rejects new bytes");
    queue.Consume(1);
    ExpectEqual(1u, calls, "final front completion notifies exactly once");
    ExpectTrue(!wait.IsPending(), "capacity wait is one-shot");
    ExpectEqual(
        std::size_t{ 0 }, queue.RetainedBytes(), "front storage releases on final completion");
...
void SendCapacitySharesBudgetWithoutMissedWake()
{
    ...
    auto firstWait = Accepted(first, 8,
        [&](Status status)
        {
            ExpectTrue(status.IsOk(), "first peer sees released capacity");
            ++firstCalls;
        });
    auto secondWait = Accepted(second, 8,
        [&](Status status)
        {
            ExpectTrue(status.IsOk(), "second peer sees released capacity");
            ++secondCalls;
        });
    producer.Consume(4);
    ExpectEqual(0u, firstCalls + secondCalls,
        "partial release does not wake queues sharing retained storage");
    producer.Consume(4);
    ...
    ExpectEqual(
        std::size_t{ 0 }, budget->UsedBytes(), "notifications themselves consume no byte capacity");
    ExpectTrue(
        !firstWait.IsPending() && !secondWait.IsPending(), "both advisory waits finish once");
    ExpectTrue(first.Send("12345678").IsOk(), "one notified producer can consume capacity");
    ExpectCode(ErrorCode::WouldBlock, second.Send("x"),
        "another producer must still retry admission normally");
    first.Clear();
    budget->Notify(); // The release has been fully dispatched before registration.
    unsigned int lateCalls = 0;
    auto late = Accepted(second, 8,
        [&](Status status)
        {
            ExpectTrue(status.IsOk(), "registration observes current capacity");
            ++lateCalls;
        });
...
void SendCapacityValidatesAndCancels()
{
    ...
    ExpectCode(ErrorCode::InvalidArgument, queue.Wait(0, rejected).GetStatus(),
        "zero requested bytes are invalid");
    ExpectCode(
        ErrorCode::InvalidArgument, queue.Wait(1, {}).GetStatus(), "empty callback is invalid");
    ExpectCode(ErrorCode::TooLarge, queue.Wait(5, rejected).GetStatus(),
        "request larger than connection limit is TooLarge");
    {
        Queue smallShared(std::make_shared<SendBudget>(3, 8));
        ExpectCode(ErrorCode::TooLarge, smallShared.Wait(4, rejected).GetStatus(),
            "request larger than shared limit is TooLarge");
    }
    unsigned int cancelledCalls = 0;
    auto cancelled = Accepted(queue, 1,
        [&](Status status)
        {
            ExpectCode(ErrorCode::Cancelled, status, "explicit cancellation reports Cancelled");
            ++cancelledCalls;
        });
    ExpectCode(ErrorCode::AlreadyExists, queue.Wait(1, rejected).GetStatus(),
        "only one pending wait is allowed");
    ExpectCode(ErrorCode::InvalidArgument, queue.Wait(0, rejected).GetStatus(),
        "argument validation precedes duplicate state");
    ExpectTrue(cancelled.Cancel(), "first explicit cancellation wins");
    ExpectTrue(!cancelled.Cancel() && !cancelled.IsPending(),
        "late cancellation cannot repeat completion");
    ExpectEqual(1u, cancelledCalls, "cancel invokes callback exactly once");
    std::stop_source parent;
    unsigned int parentCalls = 0;
    auto parentWait = Accepted(
        queue, 1,
        [&](Status status)
        {
    ...
    auto preCancelled = Accepted(
        queue, 1,
        [&](Status status)
        {
            ExpectCode(ErrorCode::Cancelled, status,
                "already-cancelled token completes during registration");
            ++parentCalls;
        },
        parent.get_token());
    ...
    auto reset = Accepted(queue, 1,
        [value = std::move(capture), &resetCalls](Status)
        {
            (void)value;
            ++resetCalls;
        });
    ...
    auto throwing = Accepted(queue, 1,
        [&](Status)
        {
            ++exceptionCalls;
    ...
    auto successor = Accepted(queue, 1,
        [&](Status status)
        {
            ExpectTrue(status.IsOk(), "registration recovers after callback exception");
            ++exceptionCalls;
        });
...
void SendCapacityCloseAndReentrancy()
{
    ...
    auto pending = Accepted(drain, 1,
        [&](Status status)
        {
            ExpectCode(ErrorCode::Closed, status,
                "closing admission completes pending capacity wait with Closed");
            ++closedCalls;
        });
    drain.Close(true);
    ExpectEqual(1u, closedCalls, "drain initiation notifies without waiting for queue completion");
    ExpectEqual(
        std::size_t{ 8 }, drain.RetainedBytes(), "drain retains outstanding payload storage");
    unsigned int rejectedCalls = 0;
    ExpectCode(ErrorCode::Closed, drain.Wait(1, [&](Status) { ++rejectedCalls; }).GetStatus(),
        "new closed wait rejects synchronously");
    ExpectCode(ErrorCode::InvalidArgument,
        drain.Wait(0, [&](Status) { ++rejectedCalls; }).GetStatus(),
        "invalid bytes precede closed state");
    ExpectCode(ErrorCode::TooLarge, drain.Wait(9, [&](Status) { ++rejectedCalls; }).GetStatus(),
        "impossible capacity precedes closed state");
    drain.Consume(8);
    drain.Close(false);
    ...
    SendCapacitySubscription ownerless;
    {
        ...
        ownerless = Accepted(destroyed, 1,
            [&](Status status)
            {
                ExpectCode(
                    ErrorCode::Closed, status, "queue destruction completes an existing wait");
                ++closedCalls;
            });
    }
    ...
    auto released = releasedDuringRegistration->Wait(
        1,
        [&](Status status)
        {
            ExpectCode(ErrorCode::Cancelled, status,
                "pre-cancelled registration invokes its cancellation callback");
    ...
    ExpectEqual(1u, destructionCalls,
        "registration survives recursive queue destruction without repeating completion");
    {
        ...
        ExpectTrue(triggering.Send("12345678").IsOk(),
            "shared budget starts full before recursive destruction");
        unsigned int triggeringCalls = 0;
        unsigned int destroyedCalls = 0;
        // Register A before B so the budget dispatcher invokes A first.
        auto triggeringWait = Accepted(triggering, 1,
            [&](Status status)
            {
                ExpectTrue(
                    status.IsOk(), "first registered queue observes released shared capacity");
                ++triggeringCalls;
                destroyedDuringNotify.reset();
            });
        auto destroyedWait = Accepted(*destroyedDuringNotify, 1,
            [&](Status status)
            {
                ExpectCode(ErrorCode::Closed, status,
                    "queue destroyed during shared dispatch still reports Closed");
                ++destroyedCalls;
            });
    ...
    first = Accepted(reentrant, 8,
        [&](Status status)
        {
            ++firstCalls;
            ExpectTrue(status.IsOk(), "reentrant callback observes capacity");
            first.Reset(); // Must not wait for itself.
            ExpectTrue(reentrant.Send("abcdefgh").IsOk(),
                "capacity callback may reenter Send outside transport lock");
            successor = Accepted(reentrant, 1,
                [&](Status next)
                {
                    ++successorCalls;
                    ExpectCode(ErrorCode::Closed, next,
                        "callback can register a successor that observes close");
                });
    ...
    ExpectTrue(!first.IsPending() && !successor.IsPending(),
        "self-reset and successor close both finish safely");
    ExpectEqual(std::size_t{ 0 }, reentrant.RetainedBytes(),
        "reentrant close releases newly enqueued bytes");
}
...
void SendCapacityResetWaitsForCallback()
{
    ...
    const std::weak_ptr<int> lifetime = capture;
    auto subscription = Accepted(queue, 1,
        [value = std::move(capture), &mutex, &wake, &entered, &release, &calls](Status status)
        {
            (void)value;
    ...
    std::thread notifying([&queue] { queue.Consume(8); });
    {
        ...
        ExpectTrue(wake.wait_for(guard, 2s, [&entered] { return entered; }),
            "callback entered on notifying thread");
    }
    ...
    ExpectTrue(finished.wait_for(30ms) == std::future_status::timeout,
        "Reset waits while callback remains active");
    {
    ...
    resetting.join();
    notifying.join();
    ExpectTrue(finished.wait_for(0ms) == std::future_status::ready,
        "Reset returns after callback completes");
...
const ServerCoreTest::CheckRegistration partial(
    "Transport.SendCapacityRetainsPartialStorage", SendCapacityRetainsPartialStorage);
const ServerCoreTest::CheckRegistration shared(
    "Transport.SendCapacitySharesBudgetWithoutMissedWake",
    SendCapacitySharesBudgetWithoutMissedWake);
const ServerCoreTest::CheckRegistration validation(
    "Transport.SendCapacityValidatesAndCancels", SendCapacityValidatesAndCancels);
const ServerCoreTest::CheckRegistration close(
    "Transport.SendCapacityCloseAndReentrancy", SendCapacityCloseAndReentrancy);
const ServerCoreTest::CheckRegistration reset(
    "Transport.SendCapacityResetWaitsForCallback", SendCapacityResetWaitsForCallback);
}
