...
struct ReentrantJob
{
    JobQueue* queue;
    int* moves;
    ReentrantJob(JobQueue& owner, int& count)
        : queue(&owner)
        , moves(&count)
    {
    }
...
void JobQueueRunsSnapshotOnly()
{
    ...
    const ServerCore::Core::Status outerPosted = queue.Post(
        [&queue, &order]()
        {
            order.push_back(1);
            const ServerCore::Core::Status innerPosted =
    ...
    ServerCoreTest::ExpectEqual(
        std::size_t{ 1 }, queue.DrainOnce(), "first drain takes its initial snapshot");
    ServerCoreTest::ExpectEqual(
        std::size_t{ 1 }, order.size(), "nested job waits for the next drain");
    ServerCoreTest::ExpectEqual(1, order[0], "first snapshot keeps insertion order");

    ServerCoreTest::ExpectEqual(
        std::size_t{ 1 }, queue.DrainOnce(), "second drain takes nested job");
...
void JobQueueAcceptsConcurrentPosts()
{
    ...
    for (int producer = 0; producer < ProducerCount; ++producer)
    {
        producers.emplace_back(
            [&queue, &executed, &everyPostSucceeded]()
            {
                for (int job = 0; job < JobsPerProducer; ++job)
                {
                    const ServerCore::Core::Status posted =
                        queue.Post([&executed]() { ++executed; });
                    if (!posted.IsOk())
                    {
    ...
    ServerCoreTest::ExpectTrue(
        everyPostSucceeded.load(std::memory_order_relaxed), "all producers post");
    ServerCoreTest::ExpectEqual(static_cast<std::size_t>(ProducerCount * JobsPerProducer),
        queue.PendingCount(), "concurrent posts all remain pending");
    ServerCoreTest::ExpectEqual(static_cast<std::size_t>(ProducerCount * JobsPerProducer),
        queue.DrainOnce(), "one drain executes all producer jobs");
    ServerCoreTest::ExpectEqual(ProducerCount * JobsPerProducer,
        executed.load(std::memory_order_relaxed), "all posted jobs execute");
}
...
void JobQueueOwnsMoveOnlyJobs()
{
    ...
    int executions = 0;
    {
        ...
        JobQueue::Job job(
            [owned = std::move(resource), identity, &executions]
            {
        ...
        ServerCoreTest::ExpectEqual(
            std::size_t{ 1 }, queue.DrainOnce(), "the owned job executes once");
        ServerCoreTest::ExpectEqual(1, executions, "draining invokes the owned callback");
        ServerCoreTest::ExpectEqual(
            1, destructions, "draining releases the executed job's resource exactly once");
        ServerCoreTest::ExpectTrue(
            queue
                .Post(
                ...
                .IsOk(),
            "a second move-only job may remain queued until destruction");
        ServerCoreTest::ExpectEqual(
            1, destructions, "a job that has not run still owns its captured resource");
    }
    ServerCoreTest::ExpectEqual(1, executions, "destroying a queue does not invoke pending work");
    ServerCoreTest::ExpectEqual(
        2, destructions, "destroying a queue releases pending move-only captures exactly once");
    JobQueue queue;
    int moves = 0;
    ...
    ServerCoreTest::ExpectEqual(
        std::size_t{ 1 }, queue.DrainOnce(), "the reentrant callable runs once");
...
void JobQueueRejectsEmptyLegacyJobs()
{
    ...
    ServerCoreTest::ExpectEqual(
        std::size_t{ 0 }, queue.PendingCount(), "empty job rejection leaves the queue unchanged");
    int executions = 0;
