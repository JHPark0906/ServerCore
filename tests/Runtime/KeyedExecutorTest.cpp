...
KeyedTaskHandle Accepted(
    KeyedExecutor& executor, std::uint64_t key, KeyedExecutor::Task task, TaskOptions options = {})
{
...
void KeyedExecutorFifoAndParallelism()
{
    ...
    auto first = Accepted(executor, 7,
    ...
    for (int index = 2; index <= 4; ++index)
        tail.push_back(Accepted(executor, 7,
    ...
    auto other = Accepted(executor, 8,
        [&](std::stop_token)
        {
...
void KeyedExecutorRoundRobin()
{
    ...
    KeyedExecutor executor;
    ExpectTrue(
        executor.Start({ 1, 3, 16, 64, 8, 32 }).IsOk(), "single-worker fair executor starts");
    auto blocker = Accepted(executor, 1,
        [gate](std::stop_token token)
        {
            gate->Enter(token);
            return Status::Ok();
    ...
    for (int index = 1; index <= 3; ++index)
        tasks.push_back(Accepted(executor, 1,
            [&, index](std::stop_token)
            {
    ...
    for (std::uint64_t key = 2; key <= 3; ++key)
        for (int index = 1; index <= 2; ++index)
            tasks.push_back(Accepted(executor, key,
                [&, key, index](std::stop_token)
                {
                    order.push_back(static_cast<int>(key) * 10 + index);
                    return Status::Ok();
                }));
    gate->Open();
...
void KeyedExecutorLimitsAndCancellation()
{
    ...
    auto pending = executor.SubmitWithCompletion(
        11,
        ...
        [&](const Status& status)
        {
            completionAfterRelease = releasedWhileCharged &&
    ...
    auto other = Accepted(
        ...
        TaskOptions{ 4 });
    ExpectTrue(executor.Submit(
                           11, [](std::stop_token) { return Status::Ok(); }, TaskOptions{ 1 })
                       .GetStatus()
                       .Code() == ErrorCode::WouldBlock,
        "per-key/global used byte capacity rejects work");
    ExpectTrue(
        executor.Submit(33, [](std::stop_token) { return Status::Ok(); }).GetStatus().Code() ==
            ErrorCode::WouldBlock,
        "active key registry has a hard cap");
    ExpectTrue(executor.Submit(
                           11, [](std::stop_token) { return Status::Ok(); }, TaskOptions{ 7 })
                       .GetStatus()
                       .Code() == ErrorCode::TooLarge,
        "one task exceeding a per-key byte cap is TooLarge");
    auto fourth = Accepted(executor, 11,
        [&](std::stop_token)
        {
            ++invoked;
            return Status::Ok();
    ...
    ExpectTrue(
        executor.Submit(11, [](std::stop_token) { return Status::Ok(); }).GetStatus().Code() ==
            ErrorCode::WouldBlock,
        "per-key outstanding count is capped before the global count is full");
    auto fifth = Accepted(executor, 22,
        [&](std::stop_token)
        {
            ++invoked;
            return Status::Ok();
    ...
    ExpectTrue(
        executor.Submit(22, [](std::stop_token) { return Status::Ok(); }).GetStatus().Code() ==
            ErrorCode::WouldBlock,
        "global outstanding count includes running work");
    if (pending.IsOk())
    {
    ...
    ExpectTrue(fourth.RequestCancel() && other.RequestCancel() && fifth.RequestCancel(),
        "remaining queued work cancels");
    ...
    options.retainedBytes = 1;
    auto reused = Accepted(
        executor, 33,
        [&](std::stop_token)
        {
            ++invoked;
    ...
    ExpectTrue(
        reused.GetToken().stop_requested(), "parent cancellation reaches the keyed task token");
    ExpectTrue(executor.Stop().IsOk(), "stop cancels and joins the occupied worker");
    Code(active, ErrorCode::Cancelled);
    ExpectTrue(executor.OutstandingCount() == 0 && executor.RetainedBytes() == 0 &&
                   executor.KeyCount() == 0,
        "shutdown releases every task, byte and registry charge");
    const auto metrics = executor.GetMetrics();
    ExpectTrue(metrics.acceptedTasks == metrics.completedTasks &&
                   metrics.cancelledTasks == metrics.completedTasks,
...
void KeyedExecutorCompletionSubscriptions()
{
    ...
    const auto task = Accepted(executor, 42,
    ...
    auto pending = task.WaitForCompletion(
        [&, task](Status status)
        {
    ...
    auto cancelledWait = task.WaitForCompletion(
        [&](Status status)
        {
    ...
    ExpectTrue(pending.IsOk() && cancelledWait.IsOk() && discarded.IsOk(),
        "keyed task accepts completion observers");
    if (discarded.IsOk())
    ...
    auto immediate = task.WaitForCompletion(
        [&, task](Status status)
        {
...
void KeyedExecutorDeadlineShutdownAndReentrancy()
{
    ...
    KeyedExecutor executor;
    ExpectTrue(!self->IsValid() && self->GetStatus().Code() == ErrorCode::InvalidArgument &&
                   !self->RequestCancel(),
        "default keyed handle is invalid");
    ExpectTrue(
        executor.Submit(0, {}).GetStatus().Code() == ErrorCode::InvalidArgument &&
            executor.Submit(0, [](std::stop_token) { return Status::Ok(); }).GetStatus().Code() ==
                ErrorCode::Closed,
    ...
    ExpectTrue(
        executor.Start({ 1, 2, 4, 16, 2, 8 }).IsOk(), "executor starts after invalid options");
    *self = Accepted(executor, 0,
        [&, gate, self](std::stop_token token)
        {
            gate->Enter(token);
            selfWaitRejected =
                self->WaitUntil(Clock::now() + 20ms).Code() == ErrorCode::InvalidArgument;
    ...
    deadline.deadline = Clock::now() + 30ms;
    auto expired = Accepted(
        executor, 0,
        [&](std::stop_token)
        {
            ++expiredInvoked;
    ...
    ExpectTrue(
        selfWaitRejected && selfStopRejected, "executor callbacks cannot self-wait or self-join");
    ExpectTrue(!self->RequestCancel() && !self->GetToken().stop_requested(),
        "late cancellation does not change a successful task token");
    auto throws = executor.SubmitWithCompletion(
        0, [](std::stop_token) -> Status { throw std::runtime_error("task failure"); },
        [](const Status&) { throw std::runtime_error("observer failure"); });
    ExpectTrue(throws.IsOk(), "throwing task is admitted");
    if (throws.IsOk())
        Code(throws.Value(), ErrorCode::PlatformError);
    ...
    ExpectTrue(
        executor.Submit(0, [](std::stop_token) { return Status::Ok(); }).GetStatus().Code() ==
                ErrorCode::Closed &&
            executor.Start().Code() == ErrorCode::Closed,
        "stopped executor cannot admit work or restart");
    const auto metrics = executor.GetMetrics();
    ExpectTrue(metrics.acceptedTasks == metrics.completedTasks && metrics.timedOutTasks == 1 &&
                   metrics.failedTasks == 1,
        "timeouts and exceptions retain distinct completed metrics");
    KeyedExecutorCompletionSubscriptions();
}

const ServerCoreTest::CheckRegistration fifo(
    "Runtime.KeyedExecutorFifoAndParallelism", KeyedExecutorFifoAndParallelism);
const ServerCoreTest::CheckRegistration fairness(
    "Runtime.KeyedExecutorRoundRobin", KeyedExecutorRoundRobin);
const ServerCoreTest::CheckRegistration limits(
    "Runtime.KeyedExecutorLimitsAndCancellation", KeyedExecutorLimitsAndCancellation);
const ServerCoreTest::CheckRegistration lifecycle(
    "Runtime.KeyedExecutorDeadlineShutdownAndReentrancy",
    KeyedExecutorDeadlineShutdownAndReentrancy);
}
