...
TaskHandle Accepted(
    TaskExecutor& executor, TaskExecutor::Task task, const TaskOptions& options = {})
{
...
void TaskExecutorBoundedAdmission()
{
    ...
    ExpectCode(
        ErrorCode::InvalidArgument, executor.Start({ 0, 2, 10 }), "zero workers are invalid");
    ExpectCode(ErrorCode::InvalidArgument, executor.Start({ 2, 0, 10 }),
        "zero pending capacity is invalid");
    ExpectCode(
        ErrorCode::InvalidArgument, executor.Start({ 2, 2, 0 }), "zero byte budget is invalid");
    ExpectTrue(
        executor.Start({ 2, 2, 10 }).IsOk(), "bounded executor starts after invalid configuration");
    ExpectCode(ErrorCode::Closed, executor.Start(), "executor cannot start twice");
    ExpectCode(
        ErrorCode::InvalidArgument, executor.Submit({}).GetStatus(), "empty function is invalid");
    ExpectCode(ErrorCode::TooLarge,
        executor.Submit([](std::stop_token) { return Status::Ok(); }, TaskOptions{ 11 })
            .GetStatus(),
        "a task exceeding total budget is TooLarge");
    Gate gate;
    const auto block = [&gate](std::stop_token)
    {
        gate.Enter();
        return Status::Ok();
    };
    ...
    const auto queued =
        Accepted(executor, [](std::stop_token) { return Status::Ok(); }, TaskOptions{ 6 });
    ...
    const auto bytesFull = executor.Submit(
        [capture = std::move(rejectedCapture)](std::stop_token)
        {
            (void)capture;
            return Status::Ok();
        },
        TaskOptions{ 1 });
    ExpectCode(
        ErrorCode::WouldBlock, bytesFull.GetStatus(), "queued plus running bytes bound admission");
    ...
    ExpectCode(ErrorCode::WouldBlock,
        executor.Submit([](std::stop_token) { return Status::Ok(); }).GetStatus(),
        "pending count also bounds zero-byte tasks");
    ExpectEqual(
        std::size_t{ 2 }, executor.PendingCount(), "only waiting tasks consume pending slots");
    ExpectEqual(std::size_t{ 10 }, executor.RetainedBytes(),
        "rejection preserves exact retained-byte total");
    ExpectCode(ErrorCode::Timeout, first.WaitUntil(Clock::now() + 20ms),
        "wait timeout bounds only observation");
    ExpectCode(ErrorCode::WouldBlock, first.GetStatus(), "wait timeout leaves task unfinished");
    ExpectTrue(
        !first.GetToken().stop_requested(), "wait timeout does not request task cancellation");
    gate.Open();
    for (const auto& handle : { first, second, queued, zeroBytes })
        ExpectTrue(
            handle.WaitUntil(Clock::now() + 2s).IsOk(), "admitted tasks complete successfully");
    ExpectEqual(
        std::size_t{ 0 }, executor.RetainedBytes(), "completion returns retained-byte capacity");
    ...
    ExpectCode(
        ErrorCode::InvalidArgument, invalid.GetStatus(), "invalid handle status is explicit");
...
void TaskExecutorQueuedCancellation()
{
    ...
    ExpectCode(ErrorCode::Cancelled, queued.WaitUntil(Clock::now() + 2s),
        "queued task cancels while worker stays busy");
    ExpectTrue(lifetime.expired(), "queued cancellation promptly releases captured resources");
    ExpectEqual(
        std::size_t{ 0 }, executor.PendingCount(), "queued cancellation returns pending slot");
    ExpectEqual(
        std::size_t{ 4 }, executor.RetainedBytes(), "only the running task remains charged");
    ...
    const auto replacement = Accepted(
        executor,
        [&executions](std::stop_token)
        {
            ++executions;
            return Status::Ok();
        },
    ...
    ExpectTrue(
        replacement.GetToken().stop_requested(), "parent cancellation reaches the task token");
    const auto preCancelled = Accepted(
        executor,
        [&executions](std::stop_token)
        {
            ++executions;
        ...
        options);
    ExpectCode(ErrorCode::Cancelled, preCancelled.WaitUntil(Clock::now() + 2s),
        "already cancelled parent skips execution");
...
void TaskExecutorDeadlinesAndExceptions()
{
    ...
    const auto queued = Accepted(
        executor,
        [&queuedRan](std::stop_token)
        {
            queuedRan = true;
            return Status::Ok();
        },
        queuedOptions);
    ExpectCode(ErrorCode::Timeout, queued.WaitUntil(Clock::now() + 2s),
        "queued deadline does not need a free worker");
    ExpectTrue(!queuedRan.load() && queued.GetToken().stop_requested(),
        "expired queued job is skipped and token stops");
    ExpectTrue(WaitForStop(running.GetToken()), "running deadline triggers cooperative stop token");
    ExpectCode(ErrorCode::WouldBlock, running.GetStatus(),
        "deadline does not claim a noncooperative job finished");
    ExpectEqual(
        std::size_t{ 5 }, executor.RetainedBytes(), "running deadline retains its memory budget");
    ExpectTrue(!running.RequestCancel(), "manual cancellation cannot replace deadline outcome");
    gate.Open();
    ExpectCode(ErrorCode::Timeout, running.WaitUntil(Clock::now() + 2s),
        "running deadline publishes Timeout after function returns");
    std::stop_source parent;
    TaskOptions parentOptions{ 2 };
    parentOptions.parentToken = parent.get_token();
    Gate cooperative;
    const auto parentTask = Accepted(
        executor,
        [&cooperative](std::stop_token token)
        {
    ...
    const auto throwing = Accepted(
        executor, [](std::stop_token) -> Status { throw std::runtime_error("task failure"); });
    ExpectCode(ErrorCode::PlatformError, throwing.WaitUntil(Clock::now() + 2s),
        "job exceptions become PlatformError");
    ...
    ExpectCode(ErrorCode::InvalidFormat, result,
        "worker survives an exception and retains application failure code");
    ExpectEqual(std::string("application status"), result.Message(),
        "application status message is preserved");
...
void TaskExecutorNestedCancellation()
{
    ...
    const auto running = Accepted(target,
        [&targetWorker](std::stop_token)
        {
            targetWorker.Enter();
            return Status::Ok();
        });
    ExpectTrue(
        targetWorker.WaitForEntered(), "target worker remains occupied during nested cancellation");
    ...
    TaskOptions options;
    options.parentToken = parent.get_token();
    const auto coordinatorTask =
        Accepted(target, [](std::stop_token) { return Status::Ok(); }, options);
    std::stop_callback holdCoordinator(
        coordinatorTask.GetToken(), [&targetCoordinator] { targetCoordinator.Enter(); });
    (void)parent.request_stop();
    ExpectTrue(targetCoordinator.WaitForEntered(),
        "target coordinator enters controlled cancellation callback");
    ...
    std::stop_callback onNestedCancel(nested.GetToken(),
        ...
        {
            sourceWaitRejected =
                own.WaitUntil(Clock::now() + 50ms).Code() == ErrorCode::InvalidArgument;
            targetWaitRejected =
                nested.WaitUntil(Clock::now() + 50ms).Code() == ErrorCode::InvalidArgument;
        });
    own = Accepted(source,
        [&](std::stop_token)
        {
    ...
    sourceWorker.Open();
    ExpectTrue(own.WaitUntil(Clock::now() + 2s).IsOk(),
        "nested cancellation returns without blocking its worker");
    ...
    ExpectTrue(sourceContextRestored.load(),
        "leaving nested cancellation restores the source worker context");
    targetCoordinator.Open();
    targetWorker.Open();
    ExpectCode(ErrorCode::Cancelled, coordinatorTask.WaitUntil(Clock::now() + 2s),
        "coordinator task completes cancellation");
    ExpectCode(ErrorCode::Cancelled, nested.WaitUntil(Clock::now() + 2s),
        "nested task completes cancellation");
    ExpectTrue(running.WaitUntil(Clock::now() + 2s).IsOk(), "target worker is released");
    ExpectTrue(
        source.Stop().IsOk() && target.Stop().IsOk(), "both nested cancellation executors join");
}
...
void TaskExecutorShutdownAndReentrancy()
{
    TaskHandle retainedHandle;
    {
        ...
        const auto queued = Accepted(
            ...
            TaskOptions{ 4 });
        gate.Open();
        ExpectCode(ErrorCode::Cancelled, own.WaitUntil(Clock::now() + 2s),
            "callback may request executor shutdown");
        ExpectCode(ErrorCode::Cancelled, queued.WaitUntil(Clock::now() + 2s),
            "shutdown cancels pending job");
        ...
        ExpectTrue(!queuedRan.load() && executor.IsStopRequested(),
            "shutdown closes admission before queued job executes");
        ExpectCode(ErrorCode::Closed,
            executor.Submit([](std::stop_token) { return Status::Ok(); }).GetStatus(),
            "stopped executor rejects new tasks");
        ExpectCode(ErrorCode::InvalidArgument, executor.Submit({}).GetStatus(),
            "invalid function precedes stopped state");
        ExpectCode(ErrorCode::TooLarge,
            executor.Submit([](std::stop_token) { return Status::Ok(); }, TaskOptions{ 9 })
                .GetStatus(),
            "oversize task precedes stopped state");
        ExpectTrue(
            executor.Stop().IsOk() && executor.Stop().IsOk(), "external stop joins idempotently");
        ExpectEqual(std::size_t{ 0 }, executor.RetainedBytes(),
            "joined executor releases all byte capacity");
        retainedHandle = own;
    }
    ExpectCode(ErrorCode::Cancelled, retainedHandle.Wait(),
        "terminal handle survives executor destruction");
    ...
    ExpectTrue(
        cancellationJoinRejected.load(), "external stop callback cannot join its own executor");
    noncooperative.RequestStop();
    ExpectCode(ErrorCode::WouldBlock, running.GetStatus(),
        "RequestStop returns before noncooperative task completes");
    gate.Open();
    ExpectTrue(noncooperative.Stop().IsOk(), "external Stop waits for cooperative release");
    ExpectCode(
        ErrorCode::Cancelled, running.GetStatus(), "shutdown outcome wins over late success");
...
void TaskExecutorCompletionSubscriptions()
{
    ...
    const auto task = Accepted(executor,
        [gate, capture](std::stop_token token)
        {
            (void)capture;
            gate->Enter(token);
            return Status::Ok();
    ...
    capture.reset();
    ExpectTrue(
        gate->WaitForEntered(), "completion subscriptions register before terminal publication");
    ...
    for (unsigned index = 0; index < 16; ++index)
    {
        ...
        if (registered.IsOk())
            subscriptions.push_back(std::move(registered.Value()));
    }
    ExpectCode(ErrorCode::WouldBlock, task.WaitForCompletion(callback).GetStatus(),
        "task bounds simultaneous terminal observers");
    if (!subscriptions.empty())
        ExpectTrue(subscriptions.front().Cancel(),
            "cancelling an observer completes it independently of its task");
    ...
    if (immediate.IsOk())
        ExpectTrue(!immediate.Value().Cancel(), "completed observer ignores late cancellation");
}
...
void TaskExecutorCompletionRace()
{
    ...
    for (unsigned int iteration = 0; iteration < 32; ++iteration)
    {
        ...
        ExpectTrue(cancelled == (result.Code() == ErrorCode::Cancelled),
            "accepted cancellation determines terminal status");
        ExpectTrue(handle.GetToken().stop_requested() == cancelled,
            "successful completion cannot acquire late cancellation");
        ...
        ExpectEqual(std::size_t{ 0 }, executor.RetainedBytes(),
            "every race returns byte capacity exactly once");
    }
...
void TaskExecutorMoveOnlyOwnership()
{
    ...
    {
        explicit ReentrantTask(ReentrantState* value)
            : state(value)
        {
        }
        ReentrantTask(ReentrantTask&& other) noexcept
            : state(other.state)
        {
    ...
    struct Capture
    {
        explicit Capture(std::atomic<int>& count)
            : released(count)
        {
        }
        ~Capture() { ++released; }
    ...
    if (completed.IsOk())
        ExpectTrue(
            completed.Value().WaitUntil(Clock::now() + 2s).IsOk(), "unique-owned task completes");
    ExpectTrue(completionAfterTaskRelease.load(), "completion observes released task captures");
    ExpectEqual(
        1, completionsReleased.load(), "terminal wait observes released completion captures");

    const auto blocked = Accepted(executor,
        [&gate](std::stop_token token)
        {
            gate.Enter(token);
            return Status::Ok();
        });
    ...
    auto cancelled = executor.SubmitWithCompletion(
        ...
        {
            (void)capture;
            completionAfterTaskRelease =
                status.Code() == ErrorCode::Cancelled && tasksReleased.load() == 2;
            ++completionsCalled;
        },
        TaskOptions{ 4 });
    ...
    ExpectTrue(completionAfterTaskRelease.load(),
        "cancel completion observes released queued task captures");
    ...
    ExpectEqual(std::size_t{ 0 }, executor.RetainedBytes(),
        "queued cancellation returns retained capacity");
    ...
    auto rejected = executor.SubmitWithCompletion(
        ...
        [capture = std::make_unique<Capture>(completionsReleased), &completionsCalled](
            const Status&)
        {
            (void)capture;
            ++completionsCalled;
        ...
        TaskOptions{ 17 });
    ExpectCode(
        ErrorCode::TooLarge, rejected.GetStatus(), "oversized move-only submission is rejected");
    ExpectEqual(3, tasksReleased.load(), "rejection releases unique task capture synchronously");
    ExpectEqual(3, completionsReleased.load(),
        "rejection releases unique completion capture synchronously");
    ...
    ExpectCode(ErrorCode::InvalidArgument,
        executor.SubmitWithCompletion(legacyTask, {}).GetStatus(),
        "empty legacy task and default completion retain InvalidArgument behavior");
    ExpectCode(ErrorCode::InvalidArgument,
        executor.SubmitWithCompletion({}, legacyCompletion).GetStatus(),
        "default task and legacy completion retain InvalidArgument behavior");
    legacyTask = [&tasksCalled](std::stop_token)
    {
        ++tasksCalled;
        return Status::Ok();
    };
    ...
    if (legacy.IsOk())
        ExpectTrue(
            legacy.Value().WaitUntil(Clock::now() + 2s).IsOk(), "mixed legacy task completes");
    auto optionalCompletion = executor.SubmitWithCompletion(
        ...
        legacyCompletion);
    ExpectTrue(
        optionalCompletion.IsOk(), "move-only task accepts an empty legacy optional completion");
...
const ServerCoreTest::CheckRegistration moveOnly(
    "Runtime.TaskExecutorMoveOnlyOwnership", TaskExecutorMoveOnlyOwnership);
const ServerCoreTest::CheckRegistration bounded(
    "Runtime.TaskExecutorBoundedAdmission", TaskExecutorBoundedAdmission);
const ServerCoreTest::CheckRegistration cancellation(
    "Runtime.TaskExecutorQueuedCancellation", TaskExecutorQueuedCancellation);
const ServerCoreTest::CheckRegistration deadlines(
    "Runtime.TaskExecutorDeadlinesAndExceptions", TaskExecutorDeadlinesAndExceptions);
const ServerCoreTest::CheckRegistration shutdown(
    "Runtime.TaskExecutorShutdownAndReentrancy", TaskExecutorShutdownAndReentrancy);
const ServerCoreTest::CheckRegistration race(
    "Runtime.TaskExecutorCompletionRace", TaskExecutorCompletionRace);
}
