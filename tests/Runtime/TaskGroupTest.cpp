...
void TaskGroupBoundsAndCollection()
{
    ...
    auto first = group.Submit([owned = std::move(capture)](std::stop_token)
    { return *owned == 42 ? Status::Ok() : Status::FailWithoutMessage(ErrorCode::InvalidArgument); }, TaskOptions{8});
    ...
    ExpectTrue(Await([&] { return group.RetainedBytes() == 0; }), "group observes true child completion");
    ExpectEqual(std::size_t{0}, group.RetainedBytes(), "completed task capture bytes returned");
    ExpectTrue(group.Submit([](std::stop_token) { return Status::Ok(); }).GetStatus().Code() == ErrorCode::WouldBlock,
    ...
    ExpectTrue(completed.IsOk() && completed.Value().size() == 1 &&
        completed.Value()[0].id == first.Value().id && completed.Value()[0].code == ErrorCode::Ok,
        "owned result collection identifies accepted child");
    auto second = group.Submit([](std::stop_token) { return Status::FailWithoutMessage(ErrorCode::NotFound); });
    ExpectTrue(second.IsOk(), "collection frees admission slot");
    std::atomic<bool> observed{false}, valid{false};
    auto subscription = group.WaitForCompletion([&](Status status)
    {
        valid = status.IsOk() && group.GetStatus().IsOk() &&
            second.Value().task.IsFinished() && group.RetainedBytes() == 0;
        observed = true;
    });
    ExpectTrue(subscription.IsOk(), "group completion observer closes admission");
    ExpectTrue(group.WaitUntil(Clock::now() + 2s).IsOk(), "group waits for completion regardless of child outcome");
    ExpectTrue(Await([&] { return observed.load(); }) && valid, "group observer sees true child terminal state");
    completed = group.TakeCompletions();
    ExpectTrue(completed.IsOk() && completed.Value().size() == 1 && completed.Value()[0].code == ErrorCode::NotFound,
        "group preserves failed child code");
    ExpectTrue(group.Submit([](std::stop_token) { return Status::Ok(); }).GetStatus().Code() == ErrorCode::Closed,
...
void TaskGroupCancellationAndDeadlines()
{
    ...
    const auto waitForCancellation = [](std::stop_token token)
    {
        ...
        while (!token.stop_requested() && Clock::now() < limit) std::this_thread::sleep_for(1ms);
        return token.stop_requested() ? Status::Ok() : Status::FailWithoutMessage(ErrorCode::InvalidArgument);
    };
    ...
    if (first.IsOk()) ExpectTrue(first.Value().task.WaitUntil(Clock::now() + 2s).Code() == ErrorCode::Cancelled,
        "child-specific token remains linked");
    if (second.IsOk()) ExpectTrue(!second.Value().task.GetToken().stop_requested(), "child cancellation does not cancel siblings");
    parent.request_stop();
    ExpectTrue(group.WaitUntil(Clock::now() + 2s).IsOk(), "parent cancellation terminates group children");
    if (second.IsOk()) ExpectTrue(second.Value().task.GetStatus().Code() == ErrorCode::Cancelled, "parent token reaches remaining child");
    ...
    if (child.IsOk()) ExpectTrue(child.Value().task.GetStatus().Code() == ErrorCode::Timeout, "group deadline retains Timeout reason");
...
void TaskGroupWaitAndShutdownRace()
{
    ...
    std::thread collector([&]
    {
        ...
        while (!collectorDone)
        {
            ...
            if (batch.IsOk()) for (const auto& completion : batch.Value())
            {
                if (!ids.insert(completion.id).second) duplicate = true;
                ++collected;
            }
    ...
    for (unsigned index = 0; index < 100; ++index)
    {
        ...
        if (child.IsOk()) ++accepted;
    }
    ExpectTrue(group.WaitUntil(Clock::now() + 2s).IsOk(), "wait races safely with completion collection");
    collectorDone = true;
    collector.join();
    auto tail = group.TakeCompletions();
    if (tail.IsOk()) collected += static_cast<unsigned>(tail.Value().size());
    ...
    auto child = self.Submit([&](std::stop_token)
    {
        rejected = self.Wait().Code() == ErrorCode::InvalidArgument &&
            self.Stop().Code() == ErrorCode::InvalidArgument;
        return Status::Ok();
    });
    if (child.IsOk()) (void)child.Value().task.WaitUntil(Clock::now() + 2s);
    ExpectTrue(rejected, "group cannot starve its own executor with wait or stop");
    ExpectTrue(self.Stop().IsOk() && executor.Stop().IsOk(), "self-wait guard permits external shutdown");
}
const ServerCoreTest::CheckRegistration groupBounds("Runtime.TaskGroupBoundsAndCollection", TaskGroupBoundsAndCollection);
const ServerCoreTest::CheckRegistration groupCancel("Runtime.TaskGroupCancellationAndDeadlines", TaskGroupCancellationAndDeadlines);
const ServerCoreTest::CheckRegistration groupRace("Runtime.TaskGroupWaitAndShutdownRace", TaskGroupWaitAndShutdownRace);
}
