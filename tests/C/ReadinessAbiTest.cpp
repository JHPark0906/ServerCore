...
Tcp TcpServer(bool start = false)
{
    ...
    if (start)
        Check(sc_tcp_server_start(value) == SC_OK, "readiness TCP owner starts");
    return server;
}
...
/// <summary>
/// 등록 상한이 찬 구독은 WouldBlock이 아니라 TooLarge로 거절된다. C의 준비 규약에서 WouldBlock은
/// "구독하고 기다려라"인데, 등록 자리가 비는 것을 알려 줄 수단이 없어 기다릴 수 없기 때문이다.
/// C 구독 도우미 셋이 각자 상한을 만난다: 원천당 살아 있는 등록 4096개(Subscribe), 최신값의
/// 대기 구독 64개(SubscribeCompletion), 우편함 용량은 ReadinessLifetimeAndRaces가 본다(MakeSubscription).
/// </summary>
void RegistrationCapsReportTooLarge()
{
    sc_latest_value* rawValue = nullptr;
    if (!Check(
            sc_latest_value_create(1024, &rawValue) == SC_OK && rawValue, "latest value creates"))
        return;
    Handle<sc_latest_value, sc_latest_value_destroy> latest(rawValue, sc_latest_value_destroy);
    auto changes = Mailbox(128);
    if (!changes)
        return;
    ...
    for (uint64_t key = 1; key <= 64; ++key)
    {
        ...
        if (!Check(sc_latest_value_subscribe(rawValue, 0, changes.get(), key, &raw) == SC_OK && raw,
                "latest value accepts its documented 64 pending subscriptions"))
            return;
        held.emplace_back(raw, sc_subscription_destroy);
    }
    sc_subscription* refused = nullptr;
    ExpectTrue(
        sc_latest_value_subscribe(rawValue, 0, changes.get(), 65, &refused) == SC_TOO_LARGE &&
            !refused,
        "a 65th latest value subscription is refused with TooLarge");
    held.clear();
    auto server = TcpServer();
    if (!server)
        return;
    auto first = Mailbox(4096), second = Mailbox(1);
    if (!first || !second)
        return;
    for (uint64_t key = 1; key <= 4096; ++key)
    {
        sc_subscription* raw = nullptr;
        if (!Check(sc_tcp_server_subscribe(server.get(), first.get(), key, &raw) == SC_OK && raw,
                "one source accepts 4096 live registrations"))
            return;
        held.emplace_back(raw, sc_subscription_destroy);
    }
    ExpectTrue(sc_tcp_server_subscribe(server.get(), second.get(), 1, &refused) == SC_TOO_LARGE &&
                   !refused,
        "a 4097th registration on one source is refused with TooLarge");
}
void ReadinessLifetimeAndRaces()
{
    ServerCoreTest::SocketRuntime sockets;
    if (!Check(sockets.IsReady(), "readiness socket runtime starts"))
        return;
    RegistrationCapsReportTooLarge();
    auto mailbox = Mailbox(2);
    auto server = TcpServer();
    if (!mailbox || !server)
        return;
    ...
    ExpectTrue(
        sc_tcp_server_subscribe(server.get(), mailbox.get(), 1, &extra) == SC_ALREADY_EXISTS &&
            !extra,
    ...
    ExpectTrue(
        sc_tcp_server_subscribe(server.get(), mailbox.get(), 3, &extra) == SC_TOO_LARGE && !extra,
        "a full mailbox refuses a registration with TooLarge");
    uint64_t key = 99;
    ExpectTrue(sc_notifier_next(mailbox.get(), 10, &key) == SC_TIMEOUT && key == 0,
        "idle native source produces no periodic readiness");
    ExpectTrue(
        sc_tcp_server_stop(server.get()) == SC_OK, "closing event queue wakes registered keys");
    one.reset();
    ...
    ExpectTrue(
        Notified(mailbox.get()) == 3, "registering an already closed source rechecks readiness");
    server.reset();
    two.reset();
    three.reset();
    ...
    std::thread waiter(
        ...
        {
            uint64_t ignored = 0;
            blocked = sc_notifier_next(mailbox.get(), UINT32_MAX, &ignored);
        });
    sc_notifier_close(mailbox.get());
    waiter.join();
    ExpectTrue(blocked == SC_CLOSED, "mailbox close releases a blocking bridge wait");
    for (unsigned iteration = 0; iteration < 32; ++iteration)
    {
        auto raceMailbox = Mailbox(1);
        auto raceServer = TcpServer();
        if (!raceMailbox || !raceServer)
            return;
        ...
        std::thread closer(
            [&]
            {
                ...
                (void)sc_tcp_server_stop(raceServer.get());
            });
        start.store(true);
        sc_subscription* raw = nullptr;
...
{
    ServerCoreTest::SocketRuntime sockets;
    if (!Check(sockets.IsReady(), "readiness integration socket runtime starts"))
        return;
    auto mailbox = Mailbox(8);
    if (!mailbox)
        return;
    ...
    if (!Check(sc_web_server_create(&options, &rawWeb) == SC_OK, "readiness web owner creates"))
        return;
    Web web(rawWeb, sc_web_server_destroy);
    if (!Check(sc_web_server_stream_route(web.get(), Bytes("POST"), Bytes("/upload"), 0) == SC_OK &&
                   sc_web_server_start(web.get()) == SC_OK,
            "readiness streaming route starts"))
        return;
    sc_subscription* raw = nullptr;
    ExpectTrue(sc_web_server_subscribe(web.get(), mailbox.get(), 10, &raw) == SC_OK,
        "web event readiness registers");
    ...
    if (!Check(sc_web_server_next(web.get(), 0, &rawEvent) == SC_OK,
            "notified request is immediately available"))
        return;
    WebEvent event(rawEvent, sc_web_event_destroy);
    requestReady.reset();
    sc_http_body* rawBody = nullptr;
    sc_http_response* rawResponse = nullptr;
    if (!Check(sc_web_event_body(event.get(), &rawBody) == SC_OK &&
                   sc_web_event_response(event.get(), &rawResponse) == SC_OK,
            "request yields independent body and response owners"))
        return;
    Body body(rawBody, sc_http_body_destroy);
    Response response(rawResponse, sc_http_response_destroy);
    raw = nullptr;
    ExpectTrue(sc_http_body_subscribe(body.get(), mailbox.get(), 11, &raw) == SC_OK,
        "pending upload readiness registers");
    Subscription bodyReady(raw, sc_subscription_destroy);
    sc_body_chunk* rawChunk = nullptr;
    ExpectTrue(sc_http_body_read(body.get(), &rawChunk) == SC_WOULD_BLOCK,
        "header-only upload is pending");
    if (!http.Send("abc"))
        return;
    ExpectTrue(Notified(mailbox.get()) == 11, "body arrival notifies its pending reader");
    if (!Check(sc_http_body_read(body.get(), &rawChunk) == SC_OK && rawChunk,
            "notified body returns an owned chunk"))
        return;
    ...
    bodyReady.reset();
    raw = nullptr;
    ExpectTrue(sc_http_body_subscribe(body.get(), mailbox.get(), 12, &raw) == SC_OK,
        "clean EOF readiness rearms");
    Subscription eofReady(raw, sc_subscription_destroy);
    ExpectTrue(
        Notified(mailbox.get()) == 12, "clean EOF is observable without waiting for another byte");
    rawChunk = nullptr;
    ExpectTrue(
        sc_http_body_read(body.get(), &rawChunk) == SC_OK && !rawChunk, "notified EOF is clean");
    eofReady.reset();
    raw = nullptr;
    ExpectTrue(
        sc_http_response_subscribe_cancelled(response.get(), mailbox.get(), 13, &raw) == SC_OK,
        "response cancellation readiness registers");
    Subscription cancelled(raw, sc_subscription_destroy);
    ExpectTrue(sc_web_server_begin_drain(web.get()) == SC_OK,
        "drain begins while response remains active");
    raw = nullptr;
    ExpectTrue(sc_web_server_subscribe_drain(web.get(), mailbox.get(), 14, &raw) == SC_OK,
        "pending drain readiness registers");
    ...
    ExpectTrue(sc_web_server_drain_status(web.get()) == SC_OK,
        "drain signal reflects terminal peer state");
    cancelled.reset();
    drained.reset();
    web.reset();

    auto tcp = TcpServer(true);
    if (!tcp)
        return;
    raw = nullptr;
    ExpectTrue(sc_tcp_server_subscribe(tcp.get(), mailbox.get(), 20, &raw) == SC_OK,
        "TCP accept readiness registers");
    Subscription accepted(raw, sc_subscription_destroy);
    Peer peer;
    if (!peer.Connect(sc_tcp_server_port(tcp.get())))
        return;
    ExpectTrue(Notified(mailbox.get()) == 20, "incoming TCP connection notifies accept");
    sc_tcp_connection* rawConnection = nullptr;
    if (!Check(sc_tcp_server_accept(tcp.get(), 0, &rawConnection) == SC_OK,
            "notified accept is nonblocking"))
        return;
    Connection connection(rawConnection, sc_tcp_connection_destroy);
    accepted.reset();
    raw = nullptr;
    ExpectTrue(sc_tcp_connection_subscribe(connection.get(), mailbox.get(), 21, &raw) == SC_OK,
        "TCP receive readiness registers");
    ...
    if (!Check(sc_tcp_connection_next(connection.get(), 0, &rawTcpEvent) == SC_OK,
            "notified TCP bytes are available"))
        return;
    TcpEvent tcpEvent(rawTcpEvent, sc_tcp_event_destroy);
    sc_wait* rawWait = nullptr;
    if (!Check(sc_tcp_connection_wait_capacity(connection.get(), 1, &rawWait) == SC_OK,
            "TCP capacity wait creates"))
        return;
    Wait capacity(rawWait, sc_wait_destroy);
    raw = nullptr;
    ExpectTrue(sc_wait_subscribe(capacity.get(), mailbox.get(), 22, &raw) == SC_OK,
        "completed capacity readiness registers");
    ...
    writable.reset();
    peer.Close();
    ExpectTrue(Notified(mailbox.get()) == 21,
        "TCP terminal event notifies the existing receive subscription");
}
const ServerCoreTest::CheckRegistration lifetime(
    "CAbi.ReadinessLifetimeAndRaces", ReadinessLifetimeAndRaces);
