...
{ return value.len ? std::string(reinterpret_cast<const char*>(value.data), value.len) : std::string{}; }
template<class T> T View()
{
    T value{}; value.abi_version = SC_ABI_VERSION; value.struct_size = sizeof(value); return value;
}
...
{ ServerCoreTest::ExpectTrue(condition, message); return condition; }
...
public:
    ...
    bool Connect(std::uint16_t port)
    {
        ...
        if (!Check(socket != ServerCoreTest::InvalidSocket, "C ABI raw peer opens")) return false;
        if (!Check(ServerCoreTest::SetSocketTimeouts(socket, 5000), "C ABI peer timeouts install")) return false;
        sockaddr_in address{}; address.sin_family = AF_INET;
        address.sin_addr.s_addr = htonl(INADDR_LOOPBACK); address.sin_port = htons(port);
        return Check(::connect(socket, reinterpret_cast<const sockaddr*>(&address), sizeof(address)) == 0,
            "C ABI raw peer connects");
    }
    void Close() { if (socket != ServerCoreTest::InvalidSocket) ServerCoreTest::CloseSocket(std::exchange(socket, ServerCoreTest::InvalidSocket)); }
    bool Send(std::string_view bytes)
    {
        while (!bytes.empty())
        {
            const auto count = ServerCoreTest::Send(socket, bytes.data(), static_cast<int>(bytes.size()));
            if (!Check(count > 0, "C ABI peer writes complete bytes")) return false;
    ...
    {
        if (!Check(count <= 1024 * 1024, "C ABI peer read allocation is bounded")) return {};
        std::string bytes(count, '\0');
        std::size_t offset = 0;
        while (offset < count)
        {
            const auto received = ServerCoreTest::Receive(socket, bytes.data() + offset, static_cast<int>(count - offset));
            if (!Check(received > 0, "C ABI peer reads the complete expected payload")) { bytes.resize(offset); return bytes; }
    ...
    std::string Head()
    {
        ...
        {
            const auto part = Read(1); if (part.empty()) break; bytes += part;
        }
        return bytes;
    }
    std::string UntilClosed(bool* closed = nullptr)
    {
        ...
        {
            const auto count = ServerCoreTest::Receive(socket, buffer.data(), static_cast<int>(buffer.size()));
            if (count <= 0)
            {
                const auto error = count < 0 ? ServerCoreTest::LastSocketError() : 0;
                const bool terminal = count == 0 ||
#ifdef _WIN32
                    error == WSAECONNRESET || error == WSAECONNABORTED || error == WSAENOTCONN;
#else
    ...
    {
        if (!Check(payload.size() < 126, "C ABI fixture uses bounded short WebSocket frames")) return false;
        std::string wire;
        ...
        for (const auto value : mask) wire += static_cast<char>(value);
    ...
    std::pair<unsigned, std::string> Frame()
    {
        ...
        if (!Check((second & 128u) == 0, "server WebSocket frames are unmasked")) return {};
        if (length == 126)
        {
            const auto extended = Read(2); if (extended.size() != 2) return {};
            length = static_cast<unsigned char>(extended[0]) * 256u + static_cast<unsigned char>(extended[1]);
        }
        if (!Check(length != 127, "C ABI fixture frame fits its bounded short reader")) return {};
        return {first & 15u, Read(length)};
    }
private:
    ServerCoreTest::Socket socket = ServerCoreTest::InvalidSocket;
};
inline std::string Http(std::string_view method, std::string_view target, std::string_view body = {})
{
