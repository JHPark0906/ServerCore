...
| 네트워크 | IPv4 TCP 수락, 부분 수신·송신, 연결별 송신 큐 | IPv6, UDP 소켓·세션 관리, TLS, HTTP/WebSocket, DNS 해석·outbound connect API |
| 프로토콜 | 길이 프레임, UTF-8 JSON, 준비된 봉투 재사용, 타입별 디스패치, 헤더 전용 DatagramCodec | 게임 스키마 자동 생성, 압축, 암호화, 큰 메시지 분할·스트리밍 |
| 세션 | ID 발급, 연결 상태, 인증 완료 상태 표시, 목록 | 계정·토큰 검증, 자동 재접속, 세션 이관·복구 |
