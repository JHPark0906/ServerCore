...
2026-09-06 샘플 제거 후 새로 내보낸 독립 소스 묶음에서 구성·빌드·회귀 테스트를 확인했다. 다른 게임·엔진 저장소, 기존 빌드 트리와 Git 이력을 포함하지 않았다. 소스 묶음은 `include/`, `src/`, `tests/`, CMake 파일과 검증 스크립트를 포함한다.
...
| --- | --- | --- | --- |
| Debug | 통과 | 115/115 | 132.11초 |
| Release | 통과 | 115/115 | 134.71초 |

총 **230/230 통과**, 실패하거나 건너뛴 단계는 없다. 기반 자료형·설정·JSON·프레이밍·세션·디스패치·실행기뿐 아니라 실제 loopback TCP 통신, 종료와 예산 회수 경로도 포함한다.

두 구성의 생성된 빌드 설정에 샘플 옵션과 실행 타깃이 없는 것도 확인했다. 라이브러리와 C++ 테스트 구현은 제거 전과 파일 해시가 같으며, 변경한 CMake 소비 검사는 위 전체 회귀에 포함된다.
...
아래는 같은 날 샘플 제거 전에 수행한 Echo 예제 검증 결과다. 이번 변경에서 README의 Echo C++ 코드가 그대로인 것을 확인했으며, 예제 실행은 다시 수행하지 않았다.
